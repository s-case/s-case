...
public class RDBMSTableColumn{
	...
	public void printTableColumn(){
		if( !this.bIsForeignKey){
			System.out.println("Column: " + this.strTableColumnName + " is added to PIM because " + this.oParentProperty.getPIMComponentProeprtyName() + " exists");
		}
		else{
			System.out.println("Column: " + this.strTableColumnName + " is added to PIM because it is foreignkey to column " + this.oReferencingTableColumn.getTableColumnName() + " that corresponds to " + this.oReferencingTableColumn.getParentProperty().getPIMComponentProeprtyName() + " property." );
		}
	}
}