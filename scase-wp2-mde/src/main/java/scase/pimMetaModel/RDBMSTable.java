...
public class RDBMSTable{
	...
	public void printRDBMSTable(){
		System.out.println("RDBMS table: " + this.strRDBMSTableName + " is added to PIM because " + this.oParentResourceModel.getResourceModelName() + " exists");
		printTableColumns();
	}
	...
	public void normalizeForeignKeyName(){
		for(int n = 0; n < this.listOfTableColumns.size(); n++){
			if((this.listOfTableColumns.get(n).getTableColumnReferentialAbility() == true) && this.listOfTableColumns.get(n).getTableColumnName().equalsIgnoreCase(this.getPrimaryKeyColumn().getTableColumnName())){
				this.listOfTableColumns.get(n).setTableColumnName(String.format("%sForeingKey", this.listOfTableColumns.get(n).getTableColumnName()));
			}
		}
	}
}