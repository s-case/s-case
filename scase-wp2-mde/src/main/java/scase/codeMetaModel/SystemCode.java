...
public class SystemCode{
	...
	private void createParentFolders(String strOutputPath, String strProjectName){
		...
		File oStructureDirectories = new File(String.format("%s/%s/src/main/java/%s", strOutputPath, strProjectName, strProjectName.toLowerCase()));
		...
		File oWebAppDirectory = new File(String.format("%s/%s/src/main/webapp/WEB-INF/classes", strOutputPath, strProjectName));
		...
		for(int n = 0; n < this.getJavaPackages().size(); n++){
			...
			if(!oPackageDirectory.exists()){
				if(!oPackageDirectory.mkdir()){
					try {
						throw new Exception("Could not create the package directory: " + this.getJavaPackages().get(n).getFileSystemPath());
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
	...
	private void createPackageFiles(){
		for(int n = 0; n < this.getJavaPackages().size(); n++){
			for(int i = 0; i < this.getJavaPackages().get(n).getPackageJavaFiles().size(); i++){
				File oPackageJavaFile = new File(String.format("%s/%s.java", this.getJavaPackages().get(n).getFileSystemPath(), this.getJavaPackages().get(n).getPackageJavaFiles().get(i).getJavaFileName()));
				try {
	...
	private void createHibernateConfigurationFile(String strOutputPath, String strProjectName){
		File oHibernateConfigurationFile = new File(String.format("%s/%s/src/main/webapp/WEB-INF/classes/hibernate.cfg.xml", strOutputPath, strProjectName));
		try {
	...
	private void createWebXMLConfigurationFile(String strOutputPath, String strProjectName){
		File oWebXMLConfigurationFile = new File(String.format("%s/%s/src/main/webapp/WEB-INF/web.xml", strOutputPath, strProjectName));
		try {
	...
	private void writeJavaFiles(String strOutputPath, String strProjectName){
		for(int n = 0; n < this.getJavaPackages().size(); n++){
			for(int i = 0; i < this.getJavaPackages().get(n).getPackageJavaFiles().size(); i++){
				File oPackageJavaFile = new File(String.format("%s/%s.java", this.getJavaPackages().get(n).getFileSystemPath(), this.getJavaPackages().get(n).getPackageJavaFiles().get(i).getJavaFileName()));
				try {
					BufferedWriter oBufferedWriter = new BufferedWriter(new FileWriter(oPackageJavaFile.getAbsoluteFile()));
		...
		File oHibernateConfigurationFile = new File(String.format("%s/%s/src/main/webapp/WEB-INF/classes/hibernate.cfg.xml", strOutputPath, strProjectName));
		try {
			BufferedWriter oBufferedWriter = new BufferedWriter(new FileWriter(oHibernateConfigurationFile.getAbsoluteFile()));
		...
		File oMavenConfigurationFile = new File(String.format("%s/%s/pom.xml", strOutputPath, strProjectName));
		try {
			BufferedWriter oBufferedWriter = new BufferedWriter(new FileWriter(oMavenConfigurationFile.getAbsoluteFile()));
		...
		File oWebXMLConfigurationFile = new File(String.format("%s/%s/src/main/webapp/WEB-INF/web.xml", strOutputPath, strProjectName));
		try {
			BufferedWriter oBufferedWriter = new BufferedWriter(new FileWriter(oWebXMLConfigurationFile.getAbsoluteFile()));
