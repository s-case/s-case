...
public class CoreCIMProducer extends ACIMProducer{
	...
	private void populateCIMResources(){
		...
		for( int n = 0; n < oSystemCIM.getListOfCIMResource().size(); n++){
			oSystemCIM.getListOfCIMResource().set(n, getInputParserImplementation().parseResourceByName(oSystemCIM.getListOfCIMResource().get(n)));
		}
	}
	...
	private void createCIMOutgoingResourceRelations(){
		...
		for( int n = 0; n < oSystemCIM.getListOfCIMResource().size(); n++){
			...
			ArrayList<String> listOfCIMResourceOutgoingRelations = getInputParserImplementation().parseResourceOutgoingRelations(oSystemCIM.getListOfCIMResource().get(n));
			...
			while(iteratorOfCIMResourceOutgoingRelations.hasNext()){
				oSystemCIM.getListOfCIMResource().set(n, oSystemCIM.getListOfCIMResource().get(n).addOutgoingRelation(getResourceIdByName(iteratorOfCIMResourceOutgoingRelations.next())));
			}
		}		
	}
	...
	private void createCIMIncomingResourceRelations(){
		...
		for( int n =0; n < oSystemCIM.getListOfCIMResource().size(); n++){
			ArrayList<String> listOfCIMResourceIncomingRelations = getInputParserImplementation().parseResourceIncomingRelations(oSystemCIM.getListOfCIMResource().get(n));
			...
			while(iteratorOfCIMResourceIncomingRelations.hasNext()){
				oSystemCIM.getListOfCIMResource().set(n, oSystemCIM.getListOfCIMResource().get(n).addIncomingRelation(getResourceIdByName(iteratorOfCIMResourceIncomingRelations.next())));
			}
		}
	}
	...
	private int getResourceIdByName(String strResourceName){
		...
		while(iteratorOfCIMResources.hasNext()){
			...
			if( oCurrentResource.getResourceName().equalsIgnoreCase(strResourceName)){//if this resource has the same name as the one we are looking for
	...
	private void createResourceURIs(){		
		for(int n = 0; n < this.oSystemCIM.getListOfCIMResource().size(); n++){
			this.oSystemCIM.getListOfCIMResource().get(n).setResourceURI(String.format("/%s", this.oSystemCIM.getListOfCIMResource().get(n).getResourceName()));
		}
	}
}