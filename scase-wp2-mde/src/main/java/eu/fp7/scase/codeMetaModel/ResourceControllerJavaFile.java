...
public class ResourceControllerJavaFile extends AJavaFile{
	...
	public ResourceControllerJavaFile(JavaResourceController oParentJavaController, String strProjectName, String strPackageStamp){
	...
	public void printJavaFile(){
		System.out.println("The Java Resource Controller File: " + this.getJavaFileName() + " is added to software code project because " +  this.oParentJavaController.getJavaResourceControllerName() + " exists in PSM");
	}
	...
	public String addClassFunctions() {
		for(int n = 0; n < this.oParentJavaController.getJavaControllerHTTPActivities().size(); n++){
			...
			if(this.oParentJavaController.getJavaControllerHTTPActivities().get(n).getParentPIMCRUDActivity().getIncomingResourceModel() != null){
				strTargetIdentifier = String.format("%s%s", (this.oParentJavaController.getPIMParentResourceController().getParentCIMResource().getResourceId() == this.oParentJavaController.getJavaControllerHTTPActivities().get(n).getParentPIMCRUDActivity().getIncomingResourceModel().getParentCIMResource().getResourceId()? "target":""), this.oParentJavaController.getRelatedJavaResourceModel().getPIMParentResourceModel().getModelPrimaryIdentifierName());
			}
			else{
				strTargetIdentifier = String.format("%s", this.oParentJavaController.getRelatedJavaResourceModel().getPIMParentResourceModel().getModelPrimaryIdentifierName());
			}
			...
			if(this.oParentJavaController.getJavaControllerHTTPActivities().get(n).getHTTPActivityVerb().equalsIgnoreCase("GET")){
				...
				if(this.oParentJavaController.getJavaControllerHTTPActivities().get(n).getParentPIMCRUDActivity().getIncomingResourceModel() != null){//if this resource is related of some other
					strSourceIdentifier = this.oParentJavaController.getJavaControllerHTTPActivities().get(n).getParentPIMCRUDActivity().getIncomingResourceModel().getModelPrimaryIdentifierName();
	...
	private HTTPActivityHandler findHTTPHandlerOfHTTPActivity(HTTPActivity oHTTPActivity){
		for(int n = 0; n < this.oParentJavaController.getJavaControllerHTTPActivityHandlers().size(); n++){
			if(this.oParentJavaController.getJavaControllerHTTPActivityHandlers().get(n).getParentPIMCRUDActivityHandler().getParentControllerCRUDActivity().getResourceControllerCRUDActivityId() == oHTTPActivity.getParentPIMCRUDActivity().getResourceControllerCRUDActivityId()){
