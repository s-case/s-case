...
public class HibernateUtilJavaFile
        extends AJavaFile
{

    private ArrayList<JavaResourceModel> listOfSystemJavaModels;

    public HibernateUtilJavaFile(String strFileName, String strProjectName, String strPackageStamp,
            ArrayList<JavaResourceModel> listOfSystemJavaModels)
    {
    ...
    {
        System.out.println("The Hibernate Utilities Java File: " + this.getJavaFileName()
                + " is added to software code project because it provides a hibernate session factory");
        super.printJavaFile();
    }
    ...
    {
        this.strFileImports = String.format("%s%simport org.hibernate.SessionFactory;%n", this.strFileImports,
                this.oJavaFileIdentation.getCurrentIdentation());
        this.strFileImports = String.format("%s%simport org.hibernate.cfg.AnnotationConfiguration;%n",
    ...
    {
        this.strClassHeader = String.format("%s%spublic class HibernateUtil{%n%n", this.strClassHeader,
                this.oJavaFileIdentation.getCurrentIdentation());
    ...
    public String addClassProperties()
    {
        this.strClassProperties = String.format(
                "%s%sprivate static final SessionFactory sessionFactory = buildSessionFactory();%n",
    ...
    {
        this.strClassFunctions = String.format("%s%sprivate static SessionFactory buildSessionFactory(){%n",
                this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation());
        this.strClassFunctions = String.format("%s%stry {%n", this.strClassFunctions,
                this.oJavaFileIdentation.increaseIdentation());
        this.strClassFunctions = String.format("%s%sreturn new AnnotationConfiguration().configure()%n",
                this.strClassFunctions, this.oJavaFileIdentation.increaseIdentation());
        this.oJavaFileIdentation.increaseIdentation();
        this.oJavaFileIdentation.increaseIdentation();
        for (int n = 0; n < this.listOfSystemJavaModels.size(); n++) {
            this.strClassFunctions = String.format("%s%s.addAnnotatedClass(%s.class)%n", this.strClassFunctions,
                    this.oJavaFileIdentation.getCurrentIdentation(), this.listOfSystemJavaModels.get(n)
                            .getJavaResourceModelName());
        }
        this.strClassFunctions = String.format("%s%s.buildSessionFactory();%n", this.strClassFunctions,
                this.oJavaFileIdentation.getCurrentIdentation());
        this.oJavaFileIdentation.decreaseIdentation();
        this.oJavaFileIdentation.decreaseIdentation();
        this.strClassFunctions = String.format("%s%s}%n", this.strClassFunctions,
                this.oJavaFileIdentation.decreaseIdentation());
        this.strClassFunctions = String.format("%s%scatch (Throwable ex){%n", this.strClassFunctions,
                this.oJavaFileIdentation.getCurrentIdentation());
        this.strClassFunctions = String.format(
                "%s%sSystem.err.println(\"Initial SessionFactory creation failed.\" + ex);%n", this.strClassFunctions,
                this.oJavaFileIdentation.increaseIdentation());
        this.strClassFunctions = String.format("%s%sthrow new ExceptionInInitializerError(ex);%n",
                this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation());
        this.strClassFunctions = String.format("%s%s}%n", this.strClassFunctions,
                this.oJavaFileIdentation.decreaseIdentation());
        this.strClassFunctions = String.format("%s%s}%n%n", this.strClassFunctions,
                this.oJavaFileIdentation.decreaseIdentation());
        this.strClassFunctions = String.format("%s%spublic static SessionFactory getSessionFactory(){%n",
                this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation());
        this.strClassFunctions = String.format("%s%sreturn sessionFactory;%n", this.strClassFunctions,
                this.oJavaFileIdentation.increaseIdentation());
        this.strClassFunctions = String.format("%s%s}%n%n", this.strClassFunctions,
                this.oJavaFileIdentation.decreaseIdentation());
