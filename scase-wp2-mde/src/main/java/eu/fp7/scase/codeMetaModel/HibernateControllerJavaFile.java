...
public class HibernateControllerJavaFile extends AJavaFile{
	...
	public String addFileImports() {
		this.strFileImports = String.format("%s%simport org.hibernate.HibernateException;\n", this.strFileImports, this.oJavaFileIdentation.getCurrentIdentation());
		...
		this.strFileImports = String.format("%s%simport org.hibernate.Transaction;\n\n", this.strFileImports, this.oJavaFileIdentation.getCurrentIdentation());
		this.strFileImports = String.format("%s%simport com.sun.jersey.core.spi.factory.ResponseBuilderImpl;\n\n", this.strFileImports, this.oJavaFileIdentation.getCurrentIdentation());
		this.strFileImports = String.format("%s%simport src.main.java.%s.utilities.HibernateUtil;\n", this.strFileImports, this.oJavaFileIdentation.getCurrentIdentation(), this.strProjectName.toLowerCase());
	...
	public String addClassHeader() {
		this.strClassHeader = String.format("%s%spublic class HibernateController{\n\n", this.strClassHeader, this.oJavaFileIdentation.getCurrentIdentation());
	...
	public String addClassProperties() {
		this.strClassProperties = String.format("%s%sprivate static HibernateController oHibernateController = new HibernateController();\n", this.strClassProperties, this.oJavaFileIdentation.getCurrentIdentation());
		...
		for(int n = 0; n < this.oParentHibernateController.getControllerHibernateActivities().size(); n++){
			if(this.oParentHibernateController.getControllerHibernateActivities().get(n).getHibernateActivityVerb().equalsIgnoreCase("POST")){
				this.strClassFunctions = String.format("%s%spublic %s %s(%s o%s){\n", this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation(),
						...
						this.oParentHibernateController.getControllerHibernateActivities().get(n).getParentJavaControllerManager().getRelatedJavaResourceModelManager().getRelatedJavaResourceModel().getJavaResourceModelName(),
						this.oParentHibernateController.getControllerHibernateActivities().get(n).getParentJavaControllerManager().getRelatedJavaResourceModelManager().getRelatedJavaResourceModel().getJavaResourceModelName());
				this.strClassFunctions = String.format("%s%sSession hibernateSession = HibernateUtil.getSessionFactory().openSession();\n", this.strClassFunctions, this.oJavaFileIdentation.increaseIdentation());
				this.strClassFunctions = String.format("%s%sTransaction hibernateTransaction = hibernateSession.beginTransaction();\n", this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation());
				this.strClassFunctions = String.format("%s%sint %s = (Integer) hibernateSession.save(o%s);\n", this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation(),
	...
	private void addUpdateActivityFunctions(){
		for(int n = 0; n < this.oParentHibernateController.getControllerHibernateActivities().size(); n++){
			if(this.oParentHibernateController.getControllerHibernateActivities().get(n).getHibernateActivityVerb().equalsIgnoreCase("PUT")){
				...
				this.strClassFunctions = String.format("%s%sSession hibernateSession = HibernateUtil.getSessionFactory().openSession();\n", this.strClassFunctions, this.oJavaFileIdentation.increaseIdentation());
				this.strClassFunctions = String.format("%s%sTransaction hibernateTransaction = hibernateSession.beginTransaction();\n", this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation());
				this.strClassFunctions = String.format("%s%shibernateSession.update(o%s);\n", this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation(),
						this.oParentHibernateController.getControllerHibernateActivities().get(n).getParentJavaResourceController().getRelatedJavaResourceModel().getJavaResourceModelName());
				this.strClassFunctions = String.format("%s%shibernateTransaction.commit();\n", this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation());
				this.strClassFunctions = String.format("%s%shibernateSession.close();\n", this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation());
				this.strClassFunctions = String.format("%s%sreturn o%s;\n", this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation(),
	...
	private void addSelectActivityFunctions(){
		for(int n = 0; n < this.oParentHibernateController.getControllerHibernateActivities().size(); n++){
			if(this.oParentHibernateController.getControllerHibernateActivities().get(n).getHibernateActivityVerb().equalsIgnoreCase("GET") &&
				...
				this.strClassFunctions = String.format("%s%sSession hibernateSession = HibernateUtil.getSessionFactory().openSession();\n", this.strClassFunctions, this.oJavaFileIdentation.increaseIdentation());
				this.strClassFunctions = String.format("%s%sTransaction hibernateTransaction = hibernateSession.beginTransaction();\n", this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation());
				this.strClassFunctions = String.format("%s%so%s = (%s) hibernateSession.get(%s.class, o%s.get%s());\n", this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation(),
						this.oParentHibernateController.getControllerHibernateActivities().get(n).getParentJavaResourceController().getRelatedJavaResourceModel().getJavaResourceModelName(),
						this.oParentHibernateController.getControllerHibernateActivities().get(n).getParentJavaResourceController().getRelatedJavaResourceModel().getJavaResourceModelName(),
						this.oParentHibernateController.getControllerHibernateActivities().get(n).getParentJavaResourceController().getRelatedJavaResourceModel().getJavaResourceModelName(),
						this.oParentHibernateController.getControllerHibernateActivities().get(n).getParentJavaResourceController().getRelatedJavaResourceModel().getJavaResourceModelName(),
		...
		for(int n = 0; n < this.oParentHibernateController.getControllerHibernateActivities().size(); n++){
			if(this.oParentHibernateController.getControllerHibernateActivities().get(n).getHibernateActivityVerb().equalsIgnoreCase("DELETE") &&
				...
				if(this.oParentHibernateController.getControllerHibernateActivities().get(n).getParentJavaResourceController().getRelatedJavaResourceModel().getRelatedJavaResourceModelManagers().size() != 0){
					this.strClassFunctions = String.format("%s%so%s.deleteAllCollections(hibernateSession);\n", this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation(),
							this.oParentHibernateController.getControllerHibernateActivities().get(n).getParentJavaResourceController().getRelatedJavaResourceModel().getJavaResourceModelName());
				}
				this.strClassFunctions = String.format("%s%shibernateSession.delete(o%s);\n", this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation(),
						this.oParentHibernateController.getControllerHibernateActivities().get(n).getParentJavaResourceController().getRelatedJavaResourceModel().getJavaResourceModelName());
				this.strClassFunctions = String.format("%s%shibernateTransaction.commit();\n", this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation());
				this.strClassFunctions = String.format("%s%shibernateSession.close();\n", this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation());
				this.strClassFunctions = String.format("%s%sreturn o%s;\n", this.strClassFunctions, this.oJavaFileIdentation.getCurrentIdentation(),
