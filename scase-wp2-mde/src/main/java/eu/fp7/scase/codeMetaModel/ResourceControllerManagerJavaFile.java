...
public class ResourceControllerManagerJavaFile
        extends AJavaFile
{

    private JavaResourceControllerManager oParentJavaControllerManager;

    public ResourceControllerManagerJavaFile(JavaResourceControllerManager oParentJavaControllerManager,
            String strProjectName, String strPackageStamp)
    {
    ...
    {
        System.out.println("The Java Resource Controller Manager File: " + this.getJavaFileName()
                + " is added to software code project because "
                + this.oParentJavaControllerManager.getJavaResourceControllerManagerName() + " exists in PSM");
    }
    ...
    public String addClassHeader()
    {
        this.strClassHeader = String.format("%s%s%n", this.oJavaFileIdentation.getCurrentIdentation(),
                this.oParentJavaControllerManager.getControllerManagerJAXRSAnnotation().getJAXRSAnnotationText());
        this.strClassHeader = String.format("%s%spublic class %s{%n%n", this.strClassHeader,
                this.oJavaFileIdentation.getCurrentIdentation(),
                this.oParentJavaControllerManager.getJavaResourceControllerManagerName());
    ...
    public String addClassFunctions()
    {
        for (int n = 0; n < this.oParentJavaControllerManager.getJavaControllerManagerHTTPActivities().size(); n++) {
            HTTPActivityHandler oHTTPActivityHandler = findHTTPHandlerOfHTTPActivity(this.oParentJavaControllerManager
                    .getJavaControllerManagerHTTPActivities().get(n));

            this.strClassFunctions = String.format("%s%s%s%n", this.strClassFunctions,
                    this.oJavaFileIdentation.getCurrentIdentation(), this.oParentJavaControllerManager
                            .getJavaControllerManagerHTTPActivities().get(n).getHTTPActivityJAXRSPathAnnotation()
                            .getJAXRSAnnotationText());
            this.strClassFunctions = String.format("%s%s%s%n", this.strClassFunctions,
                    this.oJavaFileIdentation.getCurrentIdentation(), this.oParentJavaControllerManager
                            .getJavaControllerManagerHTTPActivities().get(n).getHTTPActivityJAXRSVerbAnnotation()
                            .getJAXRSAnnotationText());
            this.strClassFunctions = String.format("%s%s%s%n", this.strClassFunctions,
                    this.oJavaFileIdentation.getCurrentIdentation(), this.oParentJavaControllerManager
                            .getJavaControllerManagerHTTPActivities().get(n).getJAXRSProduceAnnotation()
                            .getJAXRSAnnotationText());

            String strSourceIdentifier = "";
            String strSourceIdentifierFunctionSignature = "";

            if (this.oParentJavaControllerManager.getJavaControllerManagerHTTPActivities().get(n)
                    .getParentPIMCRUDActivity().getIncomingResourceModel() != null) {// if this
                                                                                     // resource is
                                                                                     // related of
                                                                                     // some other
                strSourceIdentifier = this.oParentJavaControllerManager.getJavaControllerManagerHTTPActivities().get(n)
                        .getParentPIMCRUDActivity().getIncomingResourceModel().getModelPrimaryIdentifierName();
                strSourceIdentifierFunctionSignature = String.format("@PathParam(\"%s\") int %s", strSourceIdentifier,
                        strSourceIdentifier);
            }

            if (this.oParentJavaControllerManager.getJavaControllerManagerHTTPActivities().get(n).getHTTPActivityVerb()
                ...
                this.strClassFunctions = String.format("%s%spublic %s %s(%s){%n", this.strClassFunctions,
                        ...
                        this.oParentJavaControllerManager.getJavaControllerManagerHTTPActivities().get(n)
                                .getHTTPActivityName(), strSourceIdentifierFunctionSignature);
                this.strClassFunctions = String.format("%s%s%s o%s = new %s(%soApplicationUri);%n",
                ...
                this.strClassFunctions = String.format("%s%sreturn o%s.get%s();%n", this.strClassFunctions,
                        this.oJavaFileIdentation.getCurrentIdentation(), oHTTPActivityHandler
                                .getHTTPActivityHandlerName(), this.oParentJavaControllerManager
                                .getRelatedJavaResourceModelManager().getJavaResourceModelManagerName());
            } else if (this.oParentJavaControllerManager.getJavaControllerManagerHTTPActivities().get(n)
                    .getHTTPActivityVerb().equalsIgnoreCase("POST")) {
                this.strClassFunctions = String.format("%s%s%s%n", this.strClassFunctions,
                        this.oJavaFileIdentation.getCurrentIdentation(), this.oParentJavaControllerManager
                                .getJavaControllerManagerHTTPActivities().get(n).getJAXRSConsumeAnnotation()
                                .getJAXRSAnnotationText());
                this.strClassFunctions = String.format("%s%spublic %s %s(%s%s o%s){%n", this.strClassFunctions,
                        this.oJavaFileIdentation.getCurrentIdentation(), this.oParentJavaControllerManager
                                .getRelatedJavaResourceModelManager().getRelatedJavaResourceModel()
                                .getJavaResourceModelName(), this.oParentJavaControllerManager
                                .getJavaControllerManagerHTTPActivities().get(n).getHTTPActivityName(),
                        (strSourceIdentifierFunctionSignature.equalsIgnoreCase("") ? ""
                                : strSourceIdentifierFunctionSignature + ", "), this.oParentJavaControllerManager
                                .getRelatedJavaResourceModelManager().getRelatedJavaResourceModel()
                                .getJavaResourceModelName(), this.oParentJavaControllerManager
                                .getRelatedJavaResourceModelManager().getRelatedJavaResourceModel()
                                .getJavaResourceModelName());
                this.strClassFunctions = String.format("%s%s%s o%s = new %s(%so%s, oApplicationUri);%n",
                        this.strClassFunctions, this.oJavaFileIdentation.increaseIdentation(), oHTTPActivityHandler
                                .getHTTPActivityHandlerName(), oHTTPActivityHandler.getHTTPActivityHandlerName(),
                        oHTTPActivityHandler.getHTTPActivityHandlerName(),
                        (strSourceIdentifier.equalsIgnoreCase("") ? "" : strSourceIdentifier + ", "),
                        this.oParentJavaControllerManager.getRelatedJavaResourceModelManager()
                                .getRelatedJavaResourceModel().getJavaResourceModelName());
                this.strClassFunctions = String.format("%s%sreturn o%s.post%s();%n", this.strClassFunctions,
                        this.oJavaFileIdentation.getCurrentIdentation(),
                        oHTTPActivityHandler.getHTTPActivityHandlerName(), this.oParentJavaControllerManager
                                .getRelatedJavaResourceModelManager().getRelatedJavaResourceModel()
                                .getJavaResourceModelName());
            }
    ...
    private HTTPActivityHandler findHTTPHandlerOfHTTPActivity(HTTPActivity oHTTPActivity)
    {
        for (int n = 0; n < this.oParentJavaControllerManager.getJavaControllerManagerHTTPHandlers().size(); n++) {
            if (this.oParentJavaControllerManager.getJavaControllerManagerHTTPHandlers().get(n)
