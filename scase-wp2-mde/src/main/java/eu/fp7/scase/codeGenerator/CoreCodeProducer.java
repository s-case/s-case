...
public class CoreCodeProducer
        extends ACodeProducer
{

    public CoreCodeProducer(SystemPSM oSystemPSM, String strProjectName, String strOutputPath, String strDatabaseIp,
            String strDatabasePort, String strDatabaseUsername, String strDatabasePassword)
    {
        super(oSystemPSM, strProjectName, strOutputPath, strDatabaseIp, strDatabasePort, strDatabaseUsername,
                strDatabasePassword);
    }
    ...
    private void createProjectPackages()
    {
        for (int n = 0; n < this.oSystemPSM.getPSMResourceModels().size(); n++) {
            this.oSystemCode.getJavaPackages().add(
                    new JavaPackage(this.oSystemPSM.getPSMResourceModels().get(n).getPIMParentResourceModel()
                            .getParentCIMResource(), this.oSystemCode.getSystemCodeProjectName(),
                            String.format("%s/%s/src/main/java/%s/%s", this.strOutputPath, this.strProjectName,
                                    this.strProjectName.toLowerCase(), this.oSystemPSM.getPSMResourceModels().get(n)
        ...
        for (int n = 0; n < this.oSystemPSM.getPSMAlgoResourceModels().size(); n++) {
            this.oSystemCode.getJavaPackages().add(
                    new JavaPackage(this.oSystemPSM.getPSMAlgoResourceModels().get(n).getParentPIMAlgoResourceModel()
                            .getParentCIMResource(), this.oSystemCode.getSystemCodeProjectName(), String.format(
                            "%s/%s/src/main/java/%s/%s", this.strOutputPath, this.strProjectName,
                            this.strProjectName.toLowerCase(), this.oSystemPSM.getPSMAlgoResourceModels().get(n)
                                    .getParentPIMAlgoResourceModel().getParentCIMResource().getResourceName()
                                    .toLowerCase())));
        }

        this.oSystemCode.getJavaPackages().add(
                new JavaPackage("Utilities", this.oSystemCode.getSystemCodeProjectName(), String.format(
                        "%s/%s/src/main/java/%s/utilities", this.strOutputPath, this.strProjectName,
    ...
    private void createPackageFiles()
    {
        for (int n = 0; n < this.oSystemCode.getJavaPackages().size(); n++) {
            if (this.oSystemCode.getJavaPackages().get(n).getParentResource() == null) {// if it is
                                                                                        // a custom
                                                                                        // package
                                                                                        // such as
                                                                                        // the
            ...
            } else if (!this.oSystemCode.getJavaPackages().get(n).getParentResource().getResourceType()) { // else
                                                                                                           // if
                                                                                                           // it
                                                                                                           // is
                                                                                                           // a
    ...
    private void createUtilityPackageFiles(JavaPackage oPackage)
    {
        ...
        oPackage.getPackageJavaFiles().add(
                new HibernateUtilJavaFile("HibernateUtil", this.strProjectName, oPackage.getJavaPackageStamp(),
                        this.oSystemPSM.getPSMResourceModels()));
        // add the HypermediaLink file
        oPackage.getPackageJavaFiles().add(
                new HypermediaLinkJavaFile(this.strProjectName, oPackage.getJavaPackageStamp()));
        // add the JAXRSPublisher file
        oPackage.getPackageJavaFiles().add(
                new JAXRSPublisherJavaFile("JAXRSPublisher", this.strProjectName, oPackage.getJavaPackageStamp(),
                        this.oSystemPSM.getPSMResourceControllers(),
                        this.oSystemPSM.getPSMResourceControllerManagers(), this.oSystemPSM
                                .getPSMAlgoResourceControllers()));
        // add the Hibernate Controller file
        oPackage.getPackageJavaFiles().add(
                new HibernateControllerJavaFile(this.oSystemPSM.getHibernateController(), this.strProjectName, oPackage
                        .getJavaPackageStamp(), this.oSystemPSM.getPSMResourceModels()));
    }

    private void createNonAlgorithmicPackageFiles(JavaPackage oPackage)
    {
        // add the resource controller manager file
        JavaResourceControllerManager oParentJavaControllerManager = findParentJavaControllerManager(oPackage
                .getParentResource());
        oPackage.getPackageJavaFiles().add(
                new ResourceControllerManagerJavaFile(oParentJavaControllerManager, this.strProjectName, oPackage
                        .getJavaPackageStamp()));
        ...
        oPackage.getPackageJavaFiles().add(
                new ResourceControllerJavaFile(oParentJavaController, this.strProjectName, oPackage
                        .getJavaPackageStamp()));
        // add the resource model manager file
        oPackage.getPackageJavaFiles().add(
                new ResourceModelManagerJavaFile(findParentJavaModelManager(oPackage.getParentResource()),
                        this.strProjectName, oPackage.getJavaPackageStamp()));
        // add the resource model file
        oPackage.getPackageJavaFiles().add(
                new ResourceModelJavaFile(findParentJavaModel(oPackage.getParentResource()), this.strProjectName,
                        oPackage.getJavaPackageStamp()));
        ...
        for (int n = 0; n < oParentJavaControllerManager.getJavaControllerManagerHTTPHandlers().size(); n++) {
            oPackage.getPackageJavaFiles().add(
                    new HTTPHandlerJavaFile(oParentJavaControllerManager.getJavaControllerManagerHTTPHandlers().get(n),
                            this.strProjectName, oPackage.getJavaPackageStamp()));
        }
        ...
        for (int n = 0; n < oParentJavaController.getJavaControllerHTTPActivityHandlers().size(); n++) {
            oPackage.getPackageJavaFiles().add(
                    new HTTPHandlerJavaFile(oParentJavaController.getJavaControllerHTTPActivityHandlers().get(n),
                            this.strProjectName, oPackage.getJavaPackageStamp()));
        }
    }
    ...
    private void createAlgorithmicPackageFiles(JavaPackage oPackage)
    {
        ...
        oPackage.getPackageJavaFiles().add(
                new AlgoResourceControllerJavaFile(oParentAlgoController, this.strProjectName, oPackage
                        .getJavaPackageStamp()));
        // add the algorithmic resource model file
        oPackage.getPackageJavaFiles().add(
                new AlgoResourceModelJavaFile(findParentJavaAlgoModel(oPackage.getParentResource()),
                        this.strProjectName, oPackage.getJavaPackageStamp()));
        ...
        for (int n = 0; n < oParentAlgoController.getAlgoControllerActivityHandlers().size(); n++) {
            oPackage.getPackageJavaFiles().add(
                    new HTTPHandlerJavaFile(oParentAlgoController.getAlgoControllerActivityHandlers().get(n),
                            this.strProjectName, oPackage.getJavaPackageStamp()));
        }
    }
    ...
    private JavaResourceControllerManager findParentJavaControllerManager(Resource oCIMResource)
    {
        for (int n = 0; n < this.oSystemPSM.getPSMResourceControllerManagers().size(); n++) {
            if (this.oSystemPSM.getPSMResourceControllerManagers().get(n).getPIMParentResourceControllerManager()
                    .getParentCIMResource().getResourceId() == oCIMResource.getResourceId()) {
    ...
    private JavaResourceModel findParentJavaModel(Resource oCIMResource)
    {
        for (int n = 0; n < this.oSystemPSM.getPSMResourceModels().size(); n++) {
            if (this.oSystemPSM.getPSMResourceModels().get(n).getPIMParentResourceModel().getParentCIMResource()
                    .getResourceId() == oCIMResource.getResourceId()) {
    ...
    {
        this.oSystemCode.setHibernateConfigurationFile(new HibernateConfigurationFile(this.getDatabaseIp(), this
                .getDatabasePort(), this.getDatabaseUsername(), this.getDatabasePassword()));
