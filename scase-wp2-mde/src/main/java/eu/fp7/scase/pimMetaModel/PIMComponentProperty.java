...
public class PIMComponentProperty
{
    ...
    {
        this.strResourceModelPropertyName = String.format("%s%s", this.strResourceModelPropertyName.substring(0, 1)
                .toUpperCase(), this.strResourceModelPropertyName.substring(1));
    }
    ...
    public void printPIMComponentProperty()
    {
        System.out.println("Property "
                + this.strResourceModelPropertyName
                + " with id "
                + this.iResourceModelPropertyId
                + " is added to PIM because "
                + (this.oParentProperty != null ? this.oParentProperty.getPropertyName() + " exists in CIM"
                        : " it is primary identifier or link list property"));
