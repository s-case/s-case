...
public class PSMComponentProperty
{
    ...
    {
        this.strPSMComponentPropertyName = String.format("%s%s", this.strPSMComponentPropertyName.substring(0, 1)
                .toUpperCase(), this.strPSMComponentPropertyName.substring(1));
    }
    ...
    public void printPSMComponentProperty()
    {
        System.out.println("Java Property "
                + this.strPSMComponentPropertyName
                + " with id "
                + this.iPSMComponentPropertyId
                + " is added to PSM because "
                + (this.oParentPIMComponentProperty != null ? this.oParentPIMComponentProperty
                        .getPIMComponentProeprtyName() + " exists in PIM"
                        : " because it is a PSM constraint for hibernate"));
