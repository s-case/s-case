...
public class JavaResourceModel
{
    ...
    public JavaResourceModel(ResourceModel oResourceModel) {
        ...
        this.oTableHibernateAnnotation = new HibernateAnnotation(String.format("@Table(name=\"%s\")",
                this.oPIMParentResourceModel.getParentCIMResource().getResourceName().toLowerCase()));
    ...
    public void addJavaResourceModelProperties()
    {
        for (int n = 0; n < this.oPIMParentResourceModel.getResourceModelProperties().size(); n++) {
            this.listOfJavaModelProperties.add(new JavaResourceModelProperty(this.oPIMParentResourceModel
                    .getResourceModelProperties().get(n), this));
        }
    }
    ...
    {
        System.out.println("Java Resource Model: " + this.strJavaResourceModelName + " is added to PSM because "
                + this.oPIMParentResourceModel.getResourceModelName() + " exists in PIM");
        System.out.println("JAXBAnnotation: " + this.oJAXBRootElementAnnotation.getJAXBAnnotationText());
        System.out.println("Hibernate @Entity annotation: "
                + this.oEntityHibernateAnnotation.getHibernateAnnotationText());
        System.out.println("Hibernate @Table annotation: "
                + this.oTableHibernateAnnotation.getHibernateAnnotationText());
    ...
    private void printRelatedJavaResourceModelManagers()
    {
        for (int n = 0; n < this.listOfRelatedJavaResourceModelManagers.size(); n++) {
            System.out.println("Related Java Resource Model Manager: "
                    + this.listOfRelatedJavaResourceModelManagers.get(n).getJavaResourceModelManagerName());
        }
    }
    ...
    private void printRelatedJavaAlgoResourceModels()
    {
        for (int n = 0; n < this.listOfRelatedJavaAlgoResourceModels.size(); n++) {
            System.out.println("Related Java Algorithmic Model: "
                    + this.listOfRelatedJavaAlgoResourceModels.get(n).getJavaAlgoResourceModelName());
        }
    }
    ...
    public boolean hasRelatedJavaResourceModelManager(JavaResourceModelManager oJavaResourceModelManager)
    {
        for (int n = 0; n < this.oPIMParentResourceModel.getResourceModelRelatedResourceModelManagers().size(); n++) {
            if (this.oPIMParentResourceModel.getResourceModelRelatedResourceModelManagers().get(n)
                    .getResourceModelManagerId() == oJavaResourceModelManager.getPIMParentResourceModelManager()
                    .getResourceModelManagerId()) {
                return true;
            }
        }
    ...
    public boolean hasRelatedJavaAlgoResourceModel(JavaAlgoResourceModel oJavaAlgoResourceModel)
    {
        for (int n = 0; n < this.oPIMParentResourceModel.getRelatedAlgoResourceModels().size(); n++) {
            if (this.oPIMParentResourceModel.getRelatedAlgoResourceModels().get(n).getAlgoResourceModelId() == oJavaAlgoResourceModel
                    .getParentPIMAlgoResourceModel().getAlgoResourceModelId()) {
                return true;
            }
        }
    ...
    {
        for (int n = 0; n < this.listOfRelatedJavaResourceModelManagers.size(); n++) {
            this.listOfJavaModelProperties.add(new JavaResourceModelProperty(String.format("setOf%s",
                    this.listOfRelatedJavaResourceModelManagers.get(n).getRelatedJavaResourceModel()
                            .getJavaResourceModelName()), this.listOfRelatedJavaResourceModelManagers.get(n)
                    .getRelatedJavaResourceModel().getJavaResourceModelName(), this.oPIMParentResourceModel
                    .getParentCIMResource().getResourceName()));
            this.listOfRelatedJavaResourceModelManagers
                    .get(n)
                    .getRelatedJavaResourceModel()
                    .getJavaResourceModelProperties()
                    .add(new JavaResourceModelProperty(String.format("o%s", this.oPIMParentResourceModel
                            .getParentCIMResource().getResourceName()), this.getJavaResourceModelName(),
                            (this.iJavaResourceModelId != this.getRelatedJavaResourceModelManagers().get(n)
                                    .getRelatedJavaResourceModel().getJavaResourceModelId() ? this
                                    .getPIMParentResourceModel().getModelPrimaryIdentifierName().toLowerCase() : String
                                    .format("source%s", this.listOfRelatedJavaResourceModelManagers.get(n)
                                            .getRelatedJavaResourceModel().getPIMParentResourceModel()
                                            .getModelPrimaryIdentifierName())), String.format(
                                    "fk_%s_%s",
                                    this.oPIMParentResourceModel.getParentCIMResource().getResourceName(),
                                    this.listOfRelatedJavaResourceModelManagers.get(n).getRelatedJavaResourceModel()
    ...
    {
        for (int n = 0; n < this.listOfJavaModelProperties.size(); n++) {
            if (this.listOfJavaModelProperties.get(n).getOneToManyHibernateAnnotation() != null
                    || this.listOfJavaModelProperties.get(n).getManyToOneHibernateAnnotation() != null) {
                this.listOfJavaModelSetterFunctions.add(new JavaSetterFunction(this.listOfJavaModelProperties.get(n),
                        true));
            } else {
                this.listOfJavaModelSetterFunctions.add(new JavaSetterFunction(this.listOfJavaModelProperties.get(n),
                        false));
        ...
        for (int n = 0; n < this.listOfJavaModelProperties.size(); n++) {
            if (this.listOfJavaModelProperties.get(n).getJavaModelPropertyNamingAbility()) {
                for (int i = 0; i < this.listOfJavaModelGetterFunctions.size(); i++) {
                    if (this.listOfJavaModelProperties.get(n).getPSMComponentPropertyId() == this.listOfJavaModelGetterFunctions
                            .get(i).getAccessedProperty().getPSMComponentPropertyId()) {
                        return String.format("o%s.%s()", this.strJavaResourceModelName,
                                this.listOfJavaModelGetterFunctions.get(i).getJavaFunctionName());
                    }
                }
            }
        }
