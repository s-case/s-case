...
public class HTTPActivity
{
    ...
    public HTTPActivity(ResourceControllerCRUDActivity oCRUDActivity, String strHTTPVerb, String strHTTPActivityName) {
        ...
        this.oHTTPActivityJAXRSPathAnnotation = new JAXRSAnnotation(String.format("@Path(\"%s\")",
                this.strHTTPActivityURI));
    ...
    public void createJAXRSConsumeAnnotation(ArrayList<ResourceInputRepresentation> listOfInputRepresentations)
    {
        ...
        for (int n = 0; n < listOfInputRepresentations.size(); n++) {
            if (n == 0) {
                strJAXRSConsumeAnnotation = String.format("%s\"%s\"", strJAXRSConsumeAnnotation,
                        listOfInputRepresentations.get(n).getResourceInputRepresentation());
            } else {
                strJAXRSConsumeAnnotation = String.format("%s, \"%s\"", strJAXRSConsumeAnnotation,
                        listOfInputRepresentations.get(n).getResourceInputRepresentation());
            }
    ...
    public void createJAXRSProduceAnnotaiton(ArrayList<ResourceOutputRepresentation> listOfOutputRepresentations)
    {
        ...
        for (int n = 0; n < listOfOutputRepresentations.size(); n++) {
            if (n == 0) {
                strJAXRSProduceAnnotation = String.format("%s\"%s\"", strJAXRSProduceAnnotation,
                        listOfOutputRepresentations.get(n).getResourceOutputRepresentation());
            } else {
                strJAXRSProduceAnnotation = String.format("%s, \"%s\"", strJAXRSProduceAnnotation,
                        listOfOutputRepresentations.get(n).getResourceOutputRepresentation());
            }
    ...
    {
        System.out.println("HTTP Activity: " + this.strHTTPActivityName + " is added to PSM because "
                + this.oParentPIMCRUDActivity.getCRUDActivityName() + " exists in PIM.");
