...
public class JavaHypermediaFunction
        extends JavaFunction
{
    ...
    {
        System.out.println("The Java Hypermedia Function: " + this.getJavaFunctionName() + " is added to PSM because "
                + this.oPIMParentHypermediaFunction.getPIMFunctionName() + " exists in PIM");
    ...
    private void printHypermediaLinks()
    {
        for (int n = 0; n < this.listOfPSMHypermediaLinks.size(); n++) {
            ...
            System.out.println("Links to: "
                    + (this.getPIMParentHypermediaFunction().getHypermediaLinkList().get(n)
                            .getTargetResourceController() == null ? (this.getPIMParentHypermediaFunction()
                            .getHypermediaLinkList().get(n).getTargetResourceControllerManager() == null ? this
                            .getPIMParentHypermediaFunction().getHypermediaLinkList().get(n)
                            .getTargetAlgoResourceController().getAlgoResourceControllerName() : this
                            .getPIMParentHypermediaFunction().getHypermediaLinkList().get(n)
                            .getTargetResourceControllerManager().getResourceControllerManagerName()) : this
                            .getPIMParentHypermediaFunction().getHypermediaLinkList().get(n)
                            .getTargetResourceController().getResourceControllerName()));
        }
    }
}
