...
public class HTTPActivityHandler{
	...
	private JavaResourceModel oIncomingJavaModel;//bad name, see explanation at ResourceControllerCRUDActivity.java
	
	public HTTPActivityHandler(CRUDActivityHandler oParentPIMCRUDActivityHandler, String strHTTPActivityHandlerVerb, String strHTTPActivityHandlerName, JavaResourceController oParentJavaController){
		...
		this.oCreateHypermediaFunction = new JavaHypermediaFunction(this.oParentPIMCRUDActivityHandler.getHypermediaFunction());
		transformPIMHypermediaLinks();
		capitalizeFirstNameLetter();
		this.oParentJavaController = oParentJavaController;
	}
	
	public HTTPActivityHandler(CRUDActivityHandler oParentPIMCRUDActivityHandler, String strHTTPActivityHandlerVerb, String strHTTPActivityHandlerName, JavaResourceControllerManager oParentJavaControllerManager){
		this.iHTTPActivityHandlerId = UniqueIdProducer.getNewUniqueId();
		this.oParentPIMCRUDActivityHandler = oParentPIMCRUDActivityHandler;
		this.strHTTPActivityHandlerName = strHTTPActivityHandlerName;
		this.strHTTPActivityHandlerVerb = strHTTPActivityHandlerVerb;
	...
	public HTTPActivityHandler(CRUDActivityHandler oParentPIMCRUDActivityHandler, String strHTTPActivityHandlerVerb, String strHTTPActivityHandlerName, JavaAlgoResourceController oParentAlgoJavaController){
		this.iHTTPActivityHandlerId = UniqueIdProducer.getNewUniqueId();
		this.oParentPIMCRUDActivityHandler = oParentPIMCRUDActivityHandler;
		this.strHTTPActivityHandlerName = strHTTPActivityHandlerName;
		this.strHTTPActivityHandlerVerb = strHTTPActivityHandlerVerb;
	...
	private void capitalizeFirstNameLetter(){
		this.strHTTPActivityHandlerName = String.format("%s%s", this.strHTTPActivityHandlerName.substring(0, 1).toUpperCase(), this.strHTTPActivityHandlerName.substring(1));
	}
	...
	private void transformPIMHypermediaLinks(){
		for(int n = 0; n < this.oParentPIMCRUDActivityHandler.getHypermediaFunction().getHypermediaLinkList().size(); n++){
			this.oCreateHypermediaFunction.getHypermediaLinks().add(new PSMHypermediaLink(this.oParentPIMCRUDActivityHandler.getHypermediaFunction().getHypermediaLinkList().get(n)));
		}
	}
	
	public void printHTTPActivityHandler(){
		System.out.println("The HTTP Activity Handler: " + this.strHTTPActivityHandlerName + " is added to PSM because " + this.oParentPIMCRUDActivityHandler.getCRUDActivityHandlerName() + " exists in PIM");
