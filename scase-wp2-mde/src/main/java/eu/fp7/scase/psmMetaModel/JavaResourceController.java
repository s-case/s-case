...
public class JavaResourceController{
	...
	public JavaResourceController(ResourceController oResourceController){
		...
		this.oControllerPathAnnotation = new JAXRSAnnotation(String.format("@Path(\"%s\")", this.strJavaResourceControllerURI));
	...
	public void addHTTPActivities(){
		for(int n = 0; n < this.oPIMParentResourceController.getResourceControllerCRUDActivities().size(); n++){
			this.listOfJavaControllerHTTPActivities.add(new HTTPActivity(this.oPIMParentResourceController.getResourceControllerCRUDActivities().get(n), transformCRUDVerb(this.oPIMParentResourceController.getResourceControllerCRUDActivities().get(n).getResourceControllerCRUDActivityVerb()), transformCRUDActivityName(this.oPIMParentResourceController.getResourceControllerCRUDActivities().get(n).getCRUDActivityName(), this.oPIMParentResourceController.getResourceControllerCRUDActivities().get(n).getResourceControllerCRUDActivityVerb())));
		}
	}
	...
	public void addHTTPActivityHandlers(){
		for(int n = 0; n < this.oPIMParentResourceController.getControllerCRUDActivityHandlers().size(); n++){
			this.listOfJavaControllerHTTPActivityHandlers.add(new HTTPActivityHandler(this.oPIMParentResourceController.getControllerCRUDActivityHandlers().get(n), transformCRUDVerb(this.oPIMParentResourceController.getControllerCRUDActivityHandlers().get(n).getCRUDActivityHandlerVerb()), transformCRUDActivityHandlerName(this.oPIMParentResourceController.getControllerCRUDActivityHandlers().get(n).getCRUDActivityHandlerName(), this.oPIMParentResourceController.getControllerCRUDActivityHandlers().get(n).getCRUDActivityHandlerVerb()), this));
		}
	}
	...
	public void createHibernateActivities(){
		if(hasPutHTTPActivities()){
			HibernateController.getHibernateControllerHandle().getControllerHibernateActivities().add(new HibernateActivity(String.format("put%s", this.oRelatedJavaResourceModel.getPIMParentResourceModel().getParentCIMResource().getResourceName()), "PUT", this ));
		}
		if(hasGetHTTPActivities()){
			HibernateController.getHibernateControllerHandle().getControllerHibernateActivities().add(new HibernateActivity(String.format("get%s", this.oRelatedJavaResourceModel.getPIMParentResourceModel().getParentCIMResource().getResourceName()), "GET", this ));
		}
		if(hasDeleteHTTPActivities()){
			HibernateController.getHibernateControllerHandle().getControllerHibernateActivities().add(new HibernateActivity(String.format("delete%s", this.oRelatedJavaResourceModel.getPIMParentResourceModel().getParentCIMResource().getResourceName()), "DELETE", this ));
		}		
	}
	...
	public void printJavaResourceController(){
		System.out.println("The Java Resource Controller " + this.strJavaResourceControllerName + " with related java resource model " + this.oRelatedJavaResourceModel.getJavaResourceModelName() + " is added to PSM because " + this.oPIMParentResourceController.getResourceControllerName() + " exists in PIM");
		System.out.println("URI: " + this.strJavaResourceControllerURI);
		System.out.println("JAXRS controller @PATH annotation: " + this.oControllerPathAnnotation.getJAXRSAnnotationText());
		printHTTPActivities();
	...
	public void addJAXRSInputOutputAnnotations(){
		for(int n = 0; n < this.listOfJavaControllerHTTPActivities.size(); n++){
			if( this.listOfJavaControllerHTTPActivities.get(n).getHTTPActivityVerb().equalsIgnoreCase("POST") || this.listOfJavaControllerHTTPActivities.get(n).getHTTPActivityVerb().equalsIgnoreCase("PUT")){
				this.listOfJavaControllerHTTPActivities.get(n).createJAXRSConsumeAnnotation(this.oRelatedJavaResourceModel.getPIMParentResourceModel().getResourceInputRepresentations());
			}
			this.listOfJavaControllerHTTPActivities.get(n).createJAXRSProduceAnnotaiton(this.oRelatedJavaResourceModel.getPIMParentResourceModel().getResourceOutputRepresentations());
		}
	}
