...
import eu.fp7.scase.pimMetaModel.ResourceModelProperty;

public class JavaResourceModelProperty
        extends PSMComponentProperty
{
    ...
    public JavaResourceModelProperty(ResourceModelProperty oResourceModelProperty,
            JavaResourceModel oParentResourceModel)
    {
    ...
    public JavaResourceModelProperty(String strJavaPropertyName, String strJavaPropertyType,
            String strHibernateRelationPropertyName)
    {
        super(strJavaPropertyName, strJavaPropertyType, false);
        this.bIsNamingProperty = false;
        this.bIsPrimaryIdentifier = false;
        this.oOneToManyHibernateAnnotation = new HibernateAnnotation(String.format(
                "@OneToMany(fetch = FetchType.EAGER, mappedBy=\"O%s\",orphanRemoval=true)",
                strHibernateRelationPropertyName));
        this.oOnDeleteHibernateAnnotation = new HibernateAnnotation("@OnDelete(action=OnDeleteAction.CASCADE)");
    }

    public JavaResourceModelProperty(String strJavaPropertyName, String strJavaPropertyType,
            String strIncomingJavaModelPrimardyIdentifierName, String strForeignKeyName)
    {
        ...
        this.oJoinColumnHibernateAnnotation = new HibernateAnnotation(String.format("@JoinColumn(name=\"%s\")",
                strIncomingJavaModelPrimardyIdentifierName));
        this.oForeignKeyHibernateAnnotation = new HibernateAnnotation(String.format("@ForeignKey(name = \"%s\")",
                strForeignKeyName));
    }
    ...
    private void addHibernatePropertyAnnotations()
    {
        ...
        if (isEligibleForDBColumn()) {
            this.oColumnHibernateAnnotation = new HibernateAnnotation(String.format("@Column(name = \"%s\")", this
                    .getPSMComponentPropertyName().toLowerCase()));
            if (!this.getPSMComponentPropertyUniqueness()) {
                this.oElementCollectionHibernateAnnotation = new HibernateAnnotation(
                        "@ElementCollection(fetch = FetchType.EAGER)");
                this.oCollectionTableHibernateAnnotation = new HibernateAnnotation(String.format(
                        "@CollectionTable(name=\"%s%s\", joinColumns=@JoinColumn(name=\"%s\"))",
                        this.oParentResourceModel.getPIMParentResourceModel().getParentCIMResource().getResourceName()
                                .toLowerCase(), this.oParentPIMResourceModelProperty.getParentProperty()
                                .getPropertyName().toLowerCase(), this.oParentResourceModel.getPIMParentResourceModel()
                                .getModelPrimaryIdentifierName().toLowerCase()));
                this.oForeignKeyHibernateAnnotation = new HibernateAnnotation(String.format(
                        "@ForeignKey(name = \"fk_%s_%s\")", this.oParentResourceModel.getPIMParentResourceModel()
                                .getParentCIMResource().getResourceName().toLowerCase(),
                        this.oParentPIMResourceModelProperty.getPIMComponentProeprtyName().toLowerCase()));
            }
        }
    ...
    public void printJavaResourceModelProperty()
    {
        ...
        System.out.println("Java Property naming ability: "
                + (this.bIsNamingProperty == true ? "It is naming property" : "It is not naming property"));
        System.out.println("Java Property identifying ability: "
                + (this.bIsPrimaryIdentifier == true ? "It is primary identifier" : "It is not primary identifier"));
    ...
    private void printHibernatePropertyAnnotations()
    {
        if (this.bIsPrimaryIdentifier) {
            ...
            System.out.println("Hibernate Annotation: "
                    + this.oGeneratedValueHibernateAnnotation.getHibernateAnnotationText());
        }
        ...
        if (this.isHibernateRelationColumn()) {
            if (oOneToManyHibernateAnnotation != null) {
                System.out.println("Hibernate Annotation: "
                        + this.oOneToManyHibernateAnnotation.getHibernateAnnotationText());
            }
            if (oOnDeleteHibernateAnnotation != null) {
                System.out.println("Hibernate Annotation: "
                        + this.oOnDeleteHibernateAnnotation.getHibernateAnnotationText());
            }
            if (oManyToOneHibernateAnnotation != null) {
                System.out.println("Hibernate Annotation: "
                        + this.oManyToOneHibernateAnnotation.getHibernateAnnotationText());
            }
            if (oJoinColumnHibernateAnnotation != null) {
                System.out.println("Hibernate Annotation: "
                        + this.oJoinColumnHibernateAnnotation.getHibernateAnnotationText());
            }
        }

        if (this.oElementCollectionHibernateAnnotation != null) {
            System.out.println("Hibernate Annotation: "
                    + this.oElementCollectionHibernateAnnotation.getHibernateAnnotationText());
        }

        if (this.oCollectionTableHibernateAnnotation != null) {
            System.out.println("Hibernate Annotation: "
                    + this.oCollectionTableHibernateAnnotation.getHibernateAnnotationText());
        }

        if (oForeignKeyHibernateAnnotation != null) {
            System.out.println("Hibernate Annotation: "
                    + this.oForeignKeyHibernateAnnotation.getHibernateAnnotationText());
        }

        if (this.oTransientHibernateAnnotation != null) {
            System.out.println("Hibenate Annotation: "
                    + this.oTransientHibernateAnnotation.getHibernateAnnotationText());
        }
    }
}
