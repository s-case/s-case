...
public class JavaResourceModelManager
{
    ...
    public JavaResourceModelManager(ResourceModelManager oResourceModelManager) {
        ...
        this.strJavaResourceModelManagerName = String.format("Java%s",
                oResourceModelManager.getResourceModelManagerName());
    ...
    public void addJavaResourceModelManagerProperties()
    {
        for (int n = 0; n < this.oPIMParentResourceModelManager.getResourceModelManagerProperties().size(); n++) {
            this.listOfJavaModelManagerProperties.add(new PSMComponentProperty(this.oPIMParentResourceModelManager
                    .getResourceModelManagerProperties().get(n)));
        }
    }

    public void printJavaResourceModelManager()
    {
        System.out.println("The Java Resource Model Manager: " + this.strJavaResourceModelManagerName
                + " is added to PSM because " + this.oPIMParentResourceModelManager.getResourceModelManagerName()
                + " exists in PIM");
    ...
    private void printIncomingJavaModelRelations()
    {
        for (int n = 0; n < this.listOfIncomingJavaModelRelations.size(); n++) {
            System.out.println("Is Related of "
                    + this.listOfIncomingJavaModelRelations.get(n).getJavaResourceModelName() + " Java Resource Model");
        }
    }
    ...
    {
        for (int n = 0; n < this.listOfJavaModelManagerProperties.size(); n++) {
            this.listOfJavaManagerSetterFunctions.add(new JavaSetterFunction(this.listOfJavaModelManagerProperties
                    .get(n), false));
            this.listOfJavaManagerGetterFunctions.add(new JavaGetterFunction(this.listOfJavaModelManagerProperties
                    .get(n)));
        }
    }
}
