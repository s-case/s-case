...
public class CorePIMProducer
        extends APIMProducer
{
    ...
    private void createCRUDPIMResources()
    {
        ...
        while (iteratorOfCIMResources.hasNext()) {
            ...
            if (!oCurrentCIMResource.getResourceType()) {// if this CIM resource is not algorithmic
                ResourceModel oNewResourceModel = new ResourceModel(String.format("%sModel",
                        oCurrentCIMResource.getResourceName()), oCurrentCIMResource);
                oSystemPIM.getPIMResourceModels().add(oNewResourceModel); // then add one resource
                                                                          // model to the PIM
                ResourceModelManager oNewResourceModelManager = new ResourceModelManager(String.format(
                        "%sModelManager", oCurrentCIMResource.getResourceName()), oCurrentCIMResource,
                        oNewResourceModel);
                oSystemPIM.getPIMResourceModelManagers().add(oNewResourceModelManager); // and one
                                                                                        // resource
                                                                                        // model
                                                                                        // manager
                ResourceController oNewResourceController = new ResourceController(String.format("%sController",
                        oCurrentCIMResource.getResourceName()), oCurrentCIMResource, oNewResourceModel);
                oSystemPIM.getPIMResourceControllers().add(oNewResourceController); // and one
                                                                                    // resource
                                                                                    // controller
                ResourceControllerManager oNewResourceControllerManager = new ResourceControllerManager(String.format(
                        "%sControllerManager", oCurrentCIMResource.getResourceName()), oCurrentCIMResource,
                        oNewResourceModelManager, oNewResourceController);
                oSystemPIM.getPIMResourceControllerManagers().add(oNewResourceControllerManager); // and
                                                                                                  // one
                                                                                                  // resource
                                                                                                  // controller
                                                                                                  // manager
    ...
    private void createAlgorithmicPIMResources()
    {
        ...
        while (iteratorOfCIMResource.hasNext()) {
            ...
            if (oCurrentCIMResource.getResourceType()) {// if this resource is algorithmic
                AlgoResourceModel oNewAlgoResourceModel = new AlgoResourceModel(String.format("%sAlgoModel",
                        oCurrentCIMResource.getResourceName()), oCurrentCIMResource);
                oSystemPIM.getPIMAlgoResourceModels().add(oNewAlgoResourceModel);
                AlgoResourceController oNewAlgoResourceController = new AlgoResourceController(String.format(
                        "%sAlgoController", oCurrentCIMResource.getResourceName()), oCurrentCIMResource,
                        oNewAlgoResourceModel);
    ...
    {
        for (int n = 0; n < this.oSystemPIM.getPIMResourceModels().size(); n++) {
            for (int i = 0; i < this.oSystemPIM.getPIMResourceModelManagers().size(); i++) { // add
                                                                                             // any
                                                                                             // related
                                                                                             // Resource
                                                                                             // Model
                                                                                             // Managers
                if (this.oSystemPIM.getPIMResourceModels().get(n)
                        .hasRelatedModelManager(this.oSystemPIM.getPIMResourceModelManagers().get(i))) {
                    this.oSystemPIM.getPIMResourceModels().get(n).getResourceModelRelatedResourceModelManagers()
                            .add(this.oSystemPIM.getPIMResourceModelManagers().get(i));
                }
            }

            for (int i = 0; i < this.oSystemPIM.getPIMAlgoResourceModels().size(); i++) { // add any
                                                                                          // related
                                                                                          // Algorithmic
                                                                                          // Models
                if (this.oSystemPIM.getPIMResourceModels().get(n)
                        .hasRelatedAlgoResourceModel(this.oSystemPIM.getPIMAlgoResourceModels().get(i))) {
                    this.oSystemPIM.getPIMResourceModels().get(n).getRelatedAlgoResourceModels()
                            .add(this.oSystemPIM.getPIMAlgoResourceModels().get(i));
                }
            }
        }
    }
    ...
    {
        for (int n = 0; n < this.oSystemPIM.getPIMResourceModels().size(); n++) { // add any
                                                                                  // incoming
                                                                                  // relations from
                                                                                  // Resource Models
        ...
        for (int n = 0; n < this.oSystemPIM.getPIMResourceModelManagers().size(); n++) { // add any
                                                                                         // incoming
                                                                                         // relations
                                                                                         // from
                                                                                         // Algorithmic
            ...
            for (int i = 0; i < this.oSystemPIM.getPIMAlgoResourceModels().size(); i++) {
                if (this.oSystemPIM.getPIMResourceModelManagers().get(n)
                        .isRelatedOfAlgoResourceModel(this.oSystemPIM.getPIMAlgoResourceModels().get(i))) {
                    this.oSystemPIM.getPIMResourceModelManagers().get(n).getIncomingAlgoResourceModelRelations()
                            .add(this.oSystemPIM.getPIMAlgoResourceModels().get(i));
                }
            }
        }
    ...
    {
        for (int n = 0; n < this.oSystemPIM.getPIMResourceModels().size(); n++) {
            this.oSystemPIM.getPIMRDBMSTables().add(
                    new RDBMSTable(String.format("%sTable", this.oSystemPIM.getPIMResourceModels().get(n)
                            .getParentCIMResource().getResourceName()), this.oSystemPIM.getPIMResourceModels().get(n)));
        }
    }
    ...
    private void addTableColumns()
    {
        for (int n = 0; n < this.oSystemPIM.getPIMRDBMSTables().size(); n++) {
            for (int i = 0; i < this.oSystemPIM.getPIMRDBMSTables().get(n).getRDBMSParentResource()
                    .getResourceModelProperties().size(); i++) {
                if (this.oSystemPIM.getPIMRDBMSTables().get(n).getRDBMSParentResource().getResourceModelProperties()
                        .get(i).getParentProperty() != null) { // if this property has its roots at
                                                               // CIM
                    this.oSystemPIM
                            .getPIMRDBMSTables()
                            .get(n)
                            .getTableColumns()
                            .add(new RDBMSTableColumn(this.oSystemPIM.getPIMRDBMSTables().get(n)
                                    .getRDBMSParentResource().getResourceModelProperties().get(i), false));
                }
            }
        }
    }
    ...
    private void addTablePrimaryKeys()
    {
        for (int n = 0; n < this.oSystemPIM.getPIMRDBMSTables().size(); n++) {
            for (int i = 0; i < this.oSystemPIM.getPIMRDBMSTables().get(n).getRDBMSParentResource()
                    .getResourceModelProperties().size(); i++) {
                if (this.oSystemPIM.getPIMRDBMSTables().get(n).getRDBMSParentResource().getResourceModelProperties()
                        .get(i).getResourceModelIdentifyingAbility() == true) { // if this property
                                                                                // is primary
                                                                                // identifier
                    this.oSystemPIM
                            .getPIMRDBMSTables()
    ...
    private void addTableForeignKeys()
    {
        for (int n = 0; n < this.oSystemPIM.getPIMRDBMSTables().size(); n++) {
            for (int i = 0; i < this.oSystemPIM.getPIMRDBMSTables().get(n).getRDBMSParentResource()
                    .getResourceModelRelatedResourceModelManagers().size(); i++) {
                this.oSystemPIM.getPIMRDBMSTables()
                        .get(findRelatedTable(this.oSystemPIM.getPIMRDBMSTables().get(n).getRDBMSParentResource(), i))
                        .getTableColumns()
                        .add(new RDBMSTableColumn(this.oSystemPIM.getPIMRDBMSTables().get(n).getPrimaryKeyColumn()));
            }
        }
    ...
    {
        for (int n = 0; n < this.oSystemPIM.getPIMRDBMSTables().size(); n++) {
            if (this.oSystemPIM.getPIMRDBMSTables().get(n).getRDBMSParentResource().getResourceModelId() == oResourceModel
                    .getResourceModelRelatedResourceModelManagers().get(iRelatedResourceModelManagerIndex)
                    .getRelatedResourceModel().getResourceModelId()) {
                return n;
            }
        }
    ...
    private void addAlgoResourceIncomingRelations()
    {
        ...
        for (int n = 0; n < this.oSystemPIM.getPIMAlgoResourceModels().size(); n++) { // add any
                                                                                      // incoming
                                                                                      // relations
                                                                                      // from
                                                                                      // Algorithmic
                                                                                      ...
                                                                                      // Models
            for (int i = 0; i < this.oSystemPIM.getPIMAlgoResourceModels().size(); i++) {
                if (this.oSystemPIM.getPIMAlgoResourceModels().get(n)
                        .isRelatedOfAlgoResourceModel(this.oSystemPIM.getPIMAlgoResourceModels().get(i))) {
                    this.oSystemPIM.getPIMAlgoResourceModels().get(n).getIncomingAlgoResourceModelRelations()
                            .add(this.oSystemPIM.getPIMAlgoResourceModels().get(i));
                }
            }
        }
    ...
    private void addAlgoResourceOutgoingRelations()
    {
        for (int n = 0; n < this.oSystemPIM.getPIMAlgoResourceModels().size(); n++) {
            for (int i = 0; i < this.oSystemPIM.getPIMResourceModelManagers().size(); i++) { // add
                ...
                if (this.oSystemPIM.getPIMAlgoResourceModels().get(n)
                        .hasRelatedModelManager(this.oSystemPIM.getPIMResourceModelManagers().get(i))) {
                    this.oSystemPIM.getPIMAlgoResourceModels().get(n)
                            .getAlgoResourceModelRelatedResourceModelManagers()
                            .add(this.oSystemPIM.getPIMResourceModelManagers().get(i));
                }
            }
            ...
            for (int i = 0; i < this.oSystemPIM.getPIMAlgoResourceModels().size(); i++) { // add any
                ...
                if (this.oSystemPIM.getPIMAlgoResourceModels().get(n)
                        .hasRelatedAlgoResourceModel(this.oSystemPIM.getPIMAlgoResourceModels().get(i))) {
                    this.oSystemPIM.getPIMAlgoResourceModels().get(n).getRelatedAlgoResourceModels()
                            .add(this.oSystemPIM.getPIMAlgoResourceModels().get(i));
                }
            }
        }
    ...
    {
        for (int n = 0; n < this.oSystemPIM.getPIMResourceControllerManagers().size(); n++) {
            if (!this.oSystemPIM.getPIMResourceControllerManagers().get(n).hasIncomingRelations()) { // check
                                                                                                     // if
                                                                                                     // this
                                                                                                     // controller
                                                                                                     // manager
                                                                                                     ...
                                                                                                     // other
                this.oSystemPIM
                        .getPIMResourceControllerManagers()
                        .get(n)
                        .setResourceControllerManagerURI(
                                String.format("/%s", this.oSystemPIM.getPIMResourceControllerManagers().get(n)
                                        .getParentCIMResource().getResourceName())); // create the
                                                                                     // URI of the
                                                                                     // resource
                                                                                     // controller
                                                                                     // manager
                this.oSystemPIM.getPIMResourceControllerManagers().get(n).createAssociatedResourceControllerURI(); // create
                                                                                                                   // the
                                                                                                                   // URI
                                                                                                                   // of
                                                                                                                   // the
            ...
            } else if (this.oSystemPIM.getPIMResourceControllerManagers().get(n).hasUniqueIncomingRelation()) { // else
                                                                                                                // if
                                                                                                                // it
                                                                                                                // is
                                                                                                                // related
                ...
                this.oSystemPIM
                        ...
                        .get(n)
                        .setResourceControllerManagerURI(
                                String.format("/%s/{%s}/%s", this.oSystemPIM.getPIMResourceControllerManagers().get(n)
                                        .getAssociatedResourceModelManager().getIncomingResourceModelRelations().get(0)
                                        .getParentCIMResource().getResourceName(), this.oSystemPIM
                                        .getPIMResourceControllerManagers().get(n).getAssociatedResourceModelManager()
                                        .getIncomingResourceModelRelations().get(0).getModelPrimaryIdentifierName(),
                                        this.oSystemPIM.getPIMResourceControllerManagers().get(n)
                ...
                this.oSystemPIM
                        ...
                        .get(n)
                        .setResourceControllerManagerURI(
                                String.format("/multi%sManager", this.oSystemPIM.getPIMResourceControllerManagers()
                                        .get(n).getParentCIMResource().getResourceName()));
                this.oSystemPIM
                        .getPIMResourceControllerManagers()
                        .get(n)
                        .getAssociatedResourceController()
                        .setResourceControllerURI(
                                String.format("/multi%s", this.oSystemPIM.getPIMResourceControllerManagers().get(n)
                                        .getParentCIMResource().getResourceName()));
            }
    ...
    private void createAlgoControllerURIs()
    {
        for (int n = 0; n < this.oSystemPIM.getPIMAlgoResourceControllers().size(); n++) {
            this.oSystemPIM
                    .getPIMAlgoResourceControllers()
                    .get(n)
                    .setAlgoResourceControllerURI(
                            String.format("/algo%s", this.oSystemPIM.getPIMAlgoResourceControllers().get(n)
                                    .getParentCIMResource().getResourceName()));
        }
    }
    ...
    private void createParentHypermediaLinks()
    {
        for (int n = 0; n < this.oSystemPIM.getPIMResourceControllerManagers().size(); n++) {
            if (this.oSystemPIM.getPIMResourceControllerManagers().get(n).hasIncomingRelations()) {
                for (int i = 0; i < this.oSystemPIM.getPIMResourceControllerManagers().get(n)
                        .getAssociatedResourceModelManager().getIncomingResourceModelRelations().size(); i++) {
                    this.oSystemPIM
                            .getPIMResourceControllerManagers()
                            .get(n)
                            .createSpecificParentHypermediaLinks(
                                    this.oSystemPIM.getPIMResourceControllers().get(
                                            findRelatedControllerOf(this.oSystemPIM.getPIMResourceControllerManagers()
                                                    .get(n).getAssociatedResourceModelManager()
                                                    .getIncomingResourceModelRelations().get(i))));
                }
            }
            ...
            this.oSystemPIM.getPIMResourceControllerManagers().get(n)
                    .addAssociatedResourceControllerParentHypermediaLinks();
        }

        for (int n = 0; n < this.oSystemPIM.getPIMAlgoResourceControllers().size(); n++) {
            if (this.oSystemPIM.getPIMAlgoResourceControllers().get(n).hasIncomingRelations()) {
                for (int i = 0; i < this.oSystemPIM.getPIMAlgoResourceControllers().get(n)
                        .getAssociatedAlgoResourceModel().getIncomingResourceModelRelations().size(); i++) {
                    this.oSystemPIM
                            .getPIMAlgoResourceControllers()
                            .get(n)
                            .createSpecificParentHypermediaLinks(
                                    this.oSystemPIM.getPIMResourceControllers().get(
                                            findRelatedControllerOf(this.oSystemPIM.getPIMAlgoResourceControllers()
                                                    .get(n).getAssociatedAlgoResourceModel()
                                                    .getIncomingResourceModelRelations().get(i))));
                }
            }
        }
    ...
    {
        for (int n = 0; n < this.oSystemPIM.getPIMResourceControllers().size(); n++) {
            // add any hypermedia links to related resource controller managers of this resource
            // controller
            for (int i = 0; i < this.oSystemPIM.getPIMResourceControllers().get(n).getAssociatedResourceModel()
                    .getResourceModelRelatedResourceModelManagers().size(); i++) {
                this.oSystemPIM
                        .getPIMResourceControllers()
                        .get(n)
                        .createSpecificChildHypermediaLinks(
                                this.oSystemPIM.getPIMResourceControllerManagers().get(
                                        findRelatedControllerOf(this.oSystemPIM.getPIMResourceControllers().get(n)
                                                .getAssociatedResourceModel()
                                                .getResourceModelRelatedResourceModelManagers().get(i))));
            }

            // add any hypermedia links to related algorithmic resource controllers of this resource
            // controller
            for (int i = 0; i < this.oSystemPIM.getPIMResourceControllers().get(n).getAssociatedResourceModel()
                    .getRelatedAlgoResourceModels().size(); i++) {
                this.oSystemPIM
                        .getPIMResourceControllers()
                        .get(n)
                        .createSpecificChildHypermediaLinks(
                                this.oSystemPIM.getPIMAlgoResourceControllers().get(
                                        findRelatedControllerOf(this.oSystemPIM.getPIMResourceControllers().get(n)
                                                .getAssociatedResourceModel().getRelatedAlgoResourceModels().get(i))));
            }
        }
    ...
    {
        for (int n = 0; n < this.oSystemPIM.getPIMResourceControllers().size(); n++) {
            if (this.oSystemPIM.getPIMResourceControllers().get(n).getAssociatedResourceModel().getResourceModelId() == oResourceModel
                    .getResourceModelId()) {
                return n;
            }
        }
    ...
    {
        for (int n = 0; n < this.oSystemPIM.getPIMResourceControllerManagers().size(); n++) {
            if (this.oSystemPIM.getPIMResourceControllerManagers().get(n).getAssociatedResourceModelManager()
                    .getResourceModelManagerId() == oResourceModelManager.getResourceModelManagerId()) {
                return n;
            }
        }
        ...
        try {
            throw new Exception(
                    "PIM is corrupted! Could not found the resource controller manager of a resource model manager!");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return -1;
    ...
    {
        for (int n = 0; n < this.oSystemPIM.getPIMAlgoResourceControllers().size(); n++) {
            if (this.oSystemPIM.getPIMAlgoResourceControllers().get(n).getAssociatedAlgoResourceModel()
                    .getAlgoResourceModelId() == oAlgoResourceModel.getAlgoResourceModelId()) {
                return n;
            }
        }
        ...
        try {
            throw new Exception(
                    "PIM is corrupted! Could not found the algorithmic resource controller of an algorithmic resource model manager!");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return -1;
