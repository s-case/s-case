...
public class CorePSMProducer
        extends APSMProducer
{
    ...
    private void createResourceModels()
    {
        for (int n = 0; n < this.oSystemPIM.getPIMResourceModels().size(); n++) {
            this.oSystemPSM.getPSMResourceModels().add(
                    new JavaResourceModel(this.oSystemPIM.getPIMResourceModels().get(n)));
        }
    }
    ...
    private void createResourceModelManagers()
    {
        for (int n = 0; n < this.oSystemPIM.getPIMResourceModelManagers().size(); n++) {
            this.oSystemPSM.getPSMResourceModelManagers().add(
                    new JavaResourceModelManager(this.oSystemPIM.getPIMResourceModelManagers().get(n)));
        }
    }
    ...
    private void createResourceControllerManagers()
    {
        for (int n = 0; n < this.oSystemPIM.getPIMResourceControllerManagers().size(); n++) {
            this.oSystemPSM.getPSMResourceControllerManagers().add(
                    new JavaResourceControllerManager(this.oSystemPIM.getPIMResourceControllerManagers().get(n)));
        }
    }
    ...
    private void createResourceControllers()
    {
        for (int n = 0; n < this.oSystemPIM.getPIMResourceControllers().size(); n++) {
            this.oSystemPSM.getPSMResourceControllers().add(
                    new JavaResourceController(this.oSystemPIM.getPIMResourceControllers().get(n)));
        }
    }
    ...
    private void createAlgoModels()
    {
        for (int n = 0; n < this.oSystemPIM.getPIMAlgoResourceModels().size(); n++) {
            this.oSystemPSM.getPSMAlgoResourceModels().add(
                    new JavaAlgoResourceModel(this.oSystemPIM.getPIMAlgoResourceModels().get(n)));
        }
    }
    ...
    private void createAlgoControllers()
    {
        for (int n = 0; n < this.oSystemPIM.getPIMAlgoResourceControllers().size(); n++) {
            this.oSystemPSM.getPSMAlgoResourceControllers().add(
                    new JavaAlgoResourceController(this.oSystemPIM.getPIMAlgoResourceControllers().get(n)));
        }
    }
    ...
    private void coupleControllersWithModels()
    {
        for (int n = 0; n < this.oSystemPSM.getPSMAlgoResourceControllers().size(); n++) {
            for (int i = 0; i < this.oSystemPSM.getPSMAlgoResourceModels().size(); i++) {
                if (this.oSystemPSM.getPSMAlgoResourceControllers().get(n).getParentPIMAlgoResourceController()
                        .getParentCIMResource().getResourceId() == this.oSystemPSM.getPSMAlgoResourceModels().get(i)
                        .getParentPIMAlgoResourceModel().getParentCIMResource().getResourceId()) {
                    this.oSystemPSM.getPSMAlgoResourceControllers().get(n)
                            .setRelatedAlgoResourceModel(this.oSystemPSM.getPSMAlgoResourceModels().get(i));
                    break;
                }
            }
        }
        ...
        for (int n = 0; n < this.oSystemPSM.getPSMResourceControllers().size(); n++) {
            for (int i = 0; i < this.oSystemPSM.getPSMResourceModels().size(); i++) {
                if (this.oSystemPSM.getPSMResourceControllers().get(n).getPIMParentResourceController()
                        .getParentCIMResource().getResourceId() == this.oSystemPSM.getPSMResourceModels().get(i)
                        .getPIMParentResourceModel().getParentCIMResource().getResourceId()) {
                    this.oSystemPSM.getPSMResourceControllers().get(n)
                            .setRelatedJavaResourceModel(this.oSystemPSM.getPSMResourceModels().get(i));
                    break;
                }
            }
        }
        ...
        for (int n = 0; n < this.oSystemPSM.getPSMResourceControllerManagers().size(); n++) {
            for (int i = 0; i < this.oSystemPSM.getPSMResourceModelManagers().size(); i++) {
                if (this.oSystemPSM.getPSMResourceControllerManagers().get(n).getPIMParentResourceControllerManager()
                        .getParentCIMResource().getResourceId() == this.oSystemPSM.getPSMResourceModelManagers().get(i)
                        .getPIMParentResourceModelManager().getParentCIMResource().getResourceId()) {
                    this.oSystemPSM.getPSMResourceControllerManagers().get(n)
                            .setRelatedJavaResourceModelManager(this.oSystemPSM.getPSMResourceModelManagers().get(i));
                    break;
                }
            }
        }
    ...
    private void coupleManagersWithModels()
    {
        for (int n = 0; n < this.oSystemPSM.getPSMResourceModelManagers().size(); n++) {
            for (int i = 0; i < this.oSystemPSM.getPSMResourceModels().size(); i++) {
                if (this.oSystemPSM.getPSMResourceModelManagers().get(n).getPIMParentResourceModelManager()
                        .getParentCIMResource().getResourceId() == this.oSystemPSM.getPSMResourceModels().get(i)
                        .getPIMParentResourceModel().getParentCIMResource().getResourceId()) {
                    this.oSystemPSM.getPSMResourceModelManagers().get(n)
                            .setRelatedJavaResourceModel(this.oSystemPSM.getPSMResourceModels().get(i));
                }
            }
        }
        ...
        for (int n = 0; n < this.oSystemPSM.getPSMResourceControllerManagers().size(); n++) {
            for (int i = 0; i < this.oSystemPSM.getPSMResourceControllers().size(); i++) {
                if (this.oSystemPSM.getPSMResourceControllerManagers().get(n).getPIMParentResourceControllerManager()
                        .getParentCIMResource().getResourceId() == this.oSystemPSM.getPSMResourceControllers().get(i)
                        .getPIMParentResourceController().getParentCIMResource().getResourceId()) {
                    this.oSystemPSM.getPSMResourceControllerManagers().get(n)
                            .setAssociatedJavaResourceController(this.oSystemPSM.getPSMResourceControllers().get(i));
                }
            }
        }
    }
    ...
    private void addJavaResourceModelOutgoingRelations()
    {
        ...
        for (int n = 0; n < this.oSystemPSM.getPSMResourceModels().size(); n++) {
            for (int i = 0; i < this.oSystemPSM.getPSMResourceModelManagers().size(); i++) {
                if (this.oSystemPSM.getPSMResourceModels().get(n)
                        .hasRelatedJavaResourceModelManager(this.oSystemPSM.getPSMResourceModelManagers().get(i))) {
                    this.oSystemPSM.getPSMResourceModels().get(n).getRelatedJavaResourceModelManagers()
                            .add(this.oSystemPSM.getPSMResourceModelManagers().get(i));
                }
            }
            ...
            for (int i = 0; i < this.oSystemPSM.getPSMAlgoResourceModels().size(); i++) {
                if (this.oSystemPSM.getPSMResourceModels().get(n)
                        .hasRelatedJavaAlgoResourceModel(this.oSystemPSM.getPSMAlgoResourceModels().get(i))) {
                    this.oSystemPSM.getPSMResourceModels().get(n).getRelatedJavaAlgoResourceModels()
                            .add(this.oSystemPSM.getPSMAlgoResourceModels().get(i));
                }
            }
        }
    }
    ...
    private void addJavaResourceModelManagerIncomingRelations()
    {
        ...
        for (int n = 0; n < this.oSystemPSM.getPSMResourceModelManagers().size(); n++) {
            this.oSystemPSM
                    .getPSMResourceModelManagers()
                    .get(n)
                    .getRelatedJavaResourceModel()
                    .setIncomingJavaModels(
                            this.oSystemPSM.getPSMResourceModelManagers().get(n).getIncomingJavaModelRelations());
        }
    }
    ...
    private void addHTTPActivityHandlerIncomingModels()
    {
        for (int m = 0; m < this.oSystemPSM.getPSMResourceControllerManagers().size(); m++) {
            if (this.oSystemPSM.getPSMResourceControllerManagers().get(m).getRelatedJavaResourceModelManager()
                    .getIncomingJavaModelRelations().size() != 0) {
                for (int j = 0; j < this.oSystemPSM.getPSMResourceControllerManagers().get(m)
                        .getJavaControllerManagerHTTPHandlers().size(); j++) {
                    this.oSystemPSM
                            .getPSMResourceControllerManagers()
                            .get(m)
                            .getJavaControllerManagerHTTPHandlers()
                            .get(j)
                            .setIncomingJavaModel(
                                    findIncomingJavaModel(this.oSystemPSM.getPSMResourceControllerManagers().get(m)
                                            .getJavaControllerManagerHTTPHandlers().get(j)
                                            .getParentPIMCRUDActivityHandler()));
                }

                for (int n = 0; n < this.oSystemPSM.getPSMResourceControllers().size(); n++) {
                    if (this.oSystemPSM.getPSMResourceControllers().get(n).getPIMParentResourceController()
                            .getParentCIMResource().getResourceId() == this.oSystemPSM
                            .getPSMResourceControllerManagers().get(m).getPIMParentResourceControllerManager()
                            .getParentCIMResource().getResourceId()) {
                        for (int i = 0; i < this.oSystemPSM.getPSMResourceControllers().get(n)
                                .getJavaControllerHTTPActivityHandlers().size(); i++) {
                            this.oSystemPSM
                                    .getPSMResourceControllers()
                                    .get(n)
                                    .getJavaControllerHTTPActivityHandlers()
                                    .get(i)
                                    .setIncomingJavaModel(
                                            findIncomingJavaModel(this.oSystemPSM.getPSMResourceControllers().get(n)
                                                    .getJavaControllerHTTPActivityHandlers().get(i)
                                                    .getParentPIMCRUDActivityHandler()));
                        }
                    }
                }
    ...
    {
        for (int n = 0; n < this.oSystemPSM.getPSMResourceModels().size(); n++) {
            if (this.oSystemPSM.getPSMResourceModels().get(n).getPIMParentResourceModel().getParentCIMResource()
                    .getResourceId() == oCRUDActivityHandler.getParentControllerCRUDActivity()
                    .getIncomingResourceModel().getParentCIMResource().getResourceId()) {
                return this.oSystemPSM.getPSMResourceModels().get(n);
            }
        }
        try {
            throw new Exception(
                    "Corrupted PIM! Could not find corresponding incoming java model for HTTP activity handler!");
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        for (int n = 0; n < this.oSystemPSM.getPSMResourceControllers().size(); n++) {
            for (int i = 0; i < this.oSystemPSM.getPSMResourceControllers().get(n)
                ...
                this.oSystemPSM
                        ...
                        .setReturnParameter(
                                new FunctionParameter("", findChildType(this.oSystemPSM.getPSMResourceControllers()
                                        .get(n).getJavaControllerHTTPActivityHandlers().get(i)
                                        .getHTTPHandlerHypermediaFunction().getReturnParameter().getParameterType()),
                                        true, true));
                for (int j = 0; j < this.oSystemPSM.getPSMResourceControllers().get(n)
                        .getJavaControllerHTTPActivityHandlers().get(i).getHTTPHandlerHypermediaFunction()
                        .getFunctionParameters().size(); j++) {
                    String strChildType = findChildType(this.oSystemPSM.getPSMResourceControllers().get(n)
                            .getJavaControllerHTTPActivityHandlers().get(i).getHTTPHandlerHypermediaFunction()
                            .getFunctionParameters().get(j).getParameterType());
                    this.oSystemPSM
                            .getPSMResourceControllers()
                            .get(n)
                            .getJavaControllerHTTPActivityHandlers()
                            ...
                            .set(j,
                                    new FunctionParameter(String.format("o%s", strChildType), strChildType, true, false));
                }
            }
        }

        for (int n = 0; n < this.oSystemPSM.getPSMResourceControllerManagers().size(); n++) {
            for (int i = 0; i < this.oSystemPSM.getPSMResourceControllerManagers().get(n)
                    .getJavaControllerManagerHTTPHandlers().size(); i++) {
                this.oSystemPSM
                        .getPSMResourceControllerManagers()
                        .get(n)
                        .getJavaControllerManagerHTTPHandlers()
                        ...
                        .getHTTPHandlerHypermediaFunction()
                        .setReturnParameter(
                                new FunctionParameter("", findChildType(this.oSystemPSM
                                        .getPSMResourceControllerManagers().get(n)
                                        .getJavaControllerManagerHTTPHandlers().get(i)
                                        .getHTTPHandlerHypermediaFunction().getReturnParameter().getParameterType()),
                                        true, true));
                for (int j = 0; j < this.oSystemPSM.getPSMResourceControllerManagers().get(n)
                        .getJavaControllerManagerHTTPHandlers().get(i).getHTTPHandlerHypermediaFunction()
                        .getFunctionParameters().size(); j++) {
                    String strChildType = findChildType(this.oSystemPSM.getPSMResourceControllerManagers().get(n)
                            .getJavaControllerManagerHTTPHandlers().get(i).getHTTPHandlerHypermediaFunction()
                            .getFunctionParameters().get(j).getParameterType());
                    this.oSystemPSM
                            .getPSMResourceControllerManagers()
    ...
    {
        for (int n = 0; n < this.oSystemPSM.getPSMResourceModels().size(); n++) {
            if (this.oSystemPSM.getPSMResourceModels().get(n).getPIMParentResourceModel().getResourceModelName()
                    .equalsIgnoreCase(strParentType)) {
        ...
        for (int n = 0; n < this.oSystemPSM.getPSMAlgoResourceModels().size(); n++) {
            if (this.oSystemPSM.getPSMAlgoResourceModels().get(n).getParentPIMAlgoResourceModel()
                    .getAlgoResourceModelName().equalsIgnoreCase(strParentType)) {
    ...
    {
        for (int n = 0; n < this.oSystemPSM.getPSMAlgoResourceControllers().size(); n++) {
            for (int i = 0; i < this.oSystemPSM.getPSMAlgoResourceControllers().get(n)
                    .getAlgoControllerActivityHandlers().size(); i++) {
                this.oSystemPSM
                        .getPSMAlgoResourceControllers()
                        .get(n)
                        .getAlgoControllerActivityHandlers()
                        .get(i)
                        .getHTTPHandlerHypermediaFunction()
                        .setReturnParameter(
                                new FunctionParameter("", findChildType(this.oSystemPSM.getPSMAlgoResourceControllers()
                                        .get(n).getAlgoControllerActivityHandlers().get(i)
                                        .getHTTPHandlerHypermediaFunction().getReturnParameter().getParameterType()),
                                        true, true));
                for (int j = 0; j < this.oSystemPSM.getPSMAlgoResourceControllers().get(n)
                        .getAlgoControllerActivityHandlers().get(i).getHTTPHandlerHypermediaFunction()
                        .getFunctionParameters().size(); j++) {
                    String strChildType = findChildType(this.oSystemPSM.getPSMAlgoResourceControllers().get(n)
                            .getAlgoControllerActivityHandlers().get(i).getHTTPHandlerHypermediaFunction()
                            .getFunctionParameters().get(j).getParameterType());
                    this.oSystemPSM
                            .getPSMAlgoResourceControllers()
