...
public class PreprocessAndMergeFN {
	...
	final static String[] test_files = new String[]{
			"ANC__110CYL067.xml",
			"ANC__110CYL069.xml",
			"ANC__112C-L013.xml",
	...
	final static String[] dev_files = new String[]{
	...
	public static void main(String[] args) throws Exception{
		args = new String[]{"eng",
							...
							//"-lemma", "models/lemma-eng.model", /* models trained on all CoNLL data sets (train/dev/test/ood?) */
							//"-parser", "models/parse-eng.model",
							//"-tagger", "models/tagger-eng.model",
							"-srl", "/dev/null",
							"-test", "/disk/scratch/discourse_srl/framenet/PropBank__TicketSplitting.xml"};
		...
		File[] remaining_files = new File("/disk/scratch/discourse_srl/framenet/").listFiles(new FilenameFilter(){
			public boolean accept(File arg0, String arg1) {
				if(!arg1.endsWith(".xml")) return false;
				for(String s : test_files) if(arg1.equals(s)) return false;
				for(String s : dev_files) if(arg1.equals(s)) return false;
				return true;
			}
		...
		for(int i=0; i<train_files.length; i++) train_files[i] = remaining_files[i].getName();
	...
	private static void preprocess(String[] files, String outfile) throws Exception {
