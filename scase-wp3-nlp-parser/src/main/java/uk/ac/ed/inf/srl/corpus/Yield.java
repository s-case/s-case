        ...
        implements Comparable<Yield>
{
    ...
    /**
     * Checks whether this yield is continuous, ie they contain all the words in the sentence
     * between this.first() and this.last(). Yields with 1 word are always continuous
     * 
    ...
    /**
     * Compares yields. Yields are sorted according to their left-most token. I.e. with three yields
     * with first element tokens with ID's 1, 7, and 5 respectively, they will be sorted in the
     * order 1, 5, 7, regardless of gaps and continuity
     */
    @Override
    ...
    /**
     * Breaks this yield down to continuous yields if this yield is discontinuous, otherwise it
     * returns itself in a list. Yields are labeled lab, C-lab, C-C-lab, etc in a sequential manner
     * from left to right. It follows algorithm 5.3 in Richard Johansson (2008), page 88
     * 
    ...
    public Collection<Yield> explode()
    {
        ...
        for (int i = sen.indexOf(this.first()); i <= sen.indexOf(this.last()); ++i) {
            ...
            if (this.contains(curWord)) { // If this yield contain the word, add it, it's
                                          // continuous.
                subYield.add(curWord);
            } else if (!subYield.isEmpty()) { // If this yield doesn't contain the word, and we have
                                              // an unempty subyield, then the subyield is completed
                ret.add(subYield);
