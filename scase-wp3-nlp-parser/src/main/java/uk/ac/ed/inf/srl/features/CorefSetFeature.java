        ...
        extends SetFeature
{
    ...
    private String[] makeFeatureStrings(Word arg)
    {
        ...
        for (String mention : c.chains.get(Integer.parseInt(ccid))) {
            ...
            if (snum > curr_snum || (snum == curr_snum && wnum <= curr_wnum))
                continue;
            if (snum < curr_snum - 2)
    ...
    protected void performFeatureExtraction(Sentence curr_s, boolean allWords)
    {
        ...
        /*
         * if(allWords){ for(int i=1,size=s.size();i<size;++i){ if(doExtractFeatures(s.get(i)))
         * for(Word child:s.get(i).getChildren()){ addMap(child.getAttr(attr)); } } } else {
         * for(Predicate pred:s.getPredicates()){ if(doExtractFeatures(pred)) for(Word
         * child:pred.getChildren()) addMap(child.getAttr(attr)); } }
         */
    }
}
