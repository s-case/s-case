...
public class Corpus
        extends LinkedList<CorpusSentence>
{
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                for (int i = 1; i < indices.length; i++) {
                    ...
                    CorpusSentence cs = this.get(Integer.parseInt(snum_begin_end[0]));
                    int index = cs.findhead(Integer.parseInt(snum_begin_end[1]), Integer.parseInt(snum_begin_end[2]));
                    if (index == -1) {
                        System.err.println("WARNING: No head found for coreference mention");
                        continue;
                    }
                    Word w = cs.get(index);
                    cc.add((this.indexOf(cs) + "." + cs.indexOf(w)));
                    word2chain.put((this.indexOf(cs) + "." + cs.indexOf(w)), cc);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
