package uk.ac.ed.inf.srl.options;
...
public abstract class Options {
	...
	protected void superParseCmdLine(String[] args){
		...
		ai++;
		while(ai<args.length){ //First we try to parse the common options. If none are found, then we try the specialized methods for Learn and Parse (ie the abstract parseCmdLine method)
		...
		//Ok, were done parsing the command line, now lets verify that the arguments make sense.
		if(!inputCorpus.exists() || !inputCorpus.canRead()){
			System.err.println("Input corpus"+inputCorpus+" does not exist or can not be read, aborting.");
			System.exit(1);
		}
		verifyArguments();
	}
