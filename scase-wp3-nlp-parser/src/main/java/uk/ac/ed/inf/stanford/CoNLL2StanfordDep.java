...
public class CoNLL2StanfordDep
{
            ...
            throws IOException
    {
        ...
        for (Sentence s : sentences) {
            ...
            if (((CorpusSentence) s).getMyCorpus().indexOf(s) == 0) {
                if (!lastsource.equals(""))
                    ((CorpusSentence) sentences.get(i - 1)).getMyCorpus().addCoreferenceChainsFromFile(
                            "/disk/scratch/discourse_srl/fn_coref/" + lastsource);
        ...
        // SentenceWriter writer=new CoNLL09Writer(new File("../../mate++/"+(new
        // File("Free918.parsed").getName())+"+stanforddep"));
        for (Sentence s : sentences)
    ...
    public void tag(Sentence s)
    {
        ...
        for (TypedDependency dep : getDeps(parse)) {
            ...
            // System.err.println(dep.dep().label().word() +" ("+i+") -"+ dep.reln().getShortName()
            // +"> "+ dep.gov().label().word() +" ("+head+")");
