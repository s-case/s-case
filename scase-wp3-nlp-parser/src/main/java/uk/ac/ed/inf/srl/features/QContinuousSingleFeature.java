...
public class QContinuousSingleFeature
        extends Feature
        implements QuadraticFeature
{
    ...
    protected QContinuousSingleFeature(ContinuousFeature f1, SingleFeature f2, boolean usedForPredicateIdentification,
            String POSPrefix)
    {
    ...
    protected void performFeatureExtraction(Sentence s, boolean allWords)
    {
        ...
        /*
         * if(includeArgs){ for(Predicate pred:s.getPredicates()){ if(doExtractFeatures(pred))
         * for(Word arg:pred.getArgMap().keySet()){ addMap(getFeatureString(pred,arg)); } } } else {
    ...
    {
        String ret = f1.getFeatureString(s, predIndex, argIndex) + VALUE_SEPARATOR
