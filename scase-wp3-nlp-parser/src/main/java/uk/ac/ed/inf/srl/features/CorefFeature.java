...
public class CorefFeature extends SingleFeature {
	...
	/*
	 *  Binary feature indicating whether word denotes an entity that is mentioned at several places in discourse
	 */
	...
	public String getFeatureString(Sentence s, int predIndex, int argIndex) {
		//String cc = ((CorpusSentence)s).getMyCorpus().findChainInPrvSentence(((CorpusSentence)s).getMyCorpus().corefId(s.get(argIndex)), s);
	...
	public String getFeatureString(Predicate pred, Word arg) {
		...
		//String cc = ((CorpusSentence)s).getMyCorpus().findChainInPrvSentence(((CorpusSentence)s).getMyCorpus().corefId(arg), s);
