...
public class ArgDependentEmbedding extends ContinuousFeature{
	...
	protected ArgDependentEmbedding(FeatureName name,TargetWord tw,String POSPrefix,boolean comp, boolean avg, WordEmbedding bc,int dim, boolean tokenbased) {
		...
		indices.put("WE"+(comp?"COMP":avg?"AVG":tokenbased?"TOK":TW)+dim,1);

	}
	
    @Override
    public void addFeatures(Sentence s, Collection<Integer> indices, Map<Integer, Double> nonbinFeats, int predIndex, int argIndex, Integer offset, boolean allWords) {
	nonbinFeats.put(
			indexOf(getFeatureString(s,predIndex,argIndex))+offset,
			getFeatureValue(s,predIndex,argIndex));
    }

    @Override
	public void addFeatures(Collection<Integer> indices, Map<Integer, Double> nonbinFeats, Predicate pred, Word arg, Integer offset, boolean allWords) {
	nonbinFeats.put(
			indexOf(getFeatureString(pred,arg))+offset,
	...
	if(w==null)
		...
		if(avg)
			...
			return comp?getValue(w, dim) + getValue(s.get(predIndex), dim):getValue(w, dim);
    }
    ...
    private Double getAvgFeatureValue(Word word) {
		...
		public String getFeatureString(Sentence s, int predIndex, int argIndex) {
    		return "WE"+(comp?"COMP":avg?"AVG":tokenbased?"TOK":TW)+dim;
    	}
    @Override
	...
	if(w==null)
		...
		if(avg)
			...
			return comp?getValue(w, dim) + getValue(pred, dim):getValue(w, dim);
    }
