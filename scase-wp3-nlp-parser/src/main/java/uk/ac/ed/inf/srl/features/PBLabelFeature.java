...
public class PBLabelFeature extends SingleFeature {
	...
	public PBLabelFeature(FeatureName name, TargetWord tw, boolean usedForPredicateIdentification, String POSPrefix) {
	...
	public String getFeatureString(Sentence s, int predIndex, int argIndex) {
		return ((Predicate)s.get(predIndex)).getArgMap().get(/*wordExtractor.getWord(s, predIndex, argIndex)*/s.get(argIndex));
	}
