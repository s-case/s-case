...
public class QSingleSingleFeature extends SingleFeature implements QuadraticFeature {
	...
	public String getFeatureString(Sentence s, int predIndex, int argIndex) {
		return f1.getFeatureString(s, predIndex, argIndex)+VALUE_SEPARATOR+f2.getFeatureString(s, predIndex, argIndex);
	}

	@Override
	public String getFeatureString(Predicate pred, Word arg) {
		return f1.getFeatureString(pred, arg)+VALUE_SEPARATOR+f2.getFeatureString(pred, arg);
	}
