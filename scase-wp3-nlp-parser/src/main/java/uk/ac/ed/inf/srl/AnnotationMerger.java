...
import uk.ac.ed.inf.srl.util.Relation;
import uk.ac.ed.inf.srl.util.SentenceAnnotation;
...
public class AnnotationMerger
{
    ...
    public void computeKappa()
    {
        ...
        for (String cat : labels) {
            ...
            for (int i = 0; i < instances; i++) {
                ...
                if (annotation1.get(i).equals(cat) && !annotation2.get(i).equals(cat))
                    count++;
                if (annotation2.get(i).equals(cat) && !annotation1.get(i).equals(cat))
                    count++;
            }
    ...
    private int sum_merged(String cat)
    {
        ...
        for (int i = 0; i < annotation1.size(); i++)
            if (annotation1.get(i).equals(cat) && (annotation2.get(i).equals(cat) || annotation2.get(i).equals("None"))
                    || (annotation2.get(i).equals(cat) && annotation1.get(i).equals("None")))
                retval++;
        return retval;
    }
    ...
    private int sum(List<String> annotation)
    {
        ...
        for (int i = 0; i < annotation.size(); i++)
            if (!annotation.get(i).equals("None"))
                retval++;
        return retval;
    }
    ...
    public int[] mergeAndApply(SentenceAnnotation anno1, SentenceAnnotation anno2, Sentence sen)
    {
        ...
        if (entity_agreement) {
            ...
            for (int i = 1; i < sen_size; i++) {
                ...
                for (Relation r : relations1) {
                    // if(r.head==i) { System.err.println(sen.get(i).getForm() + " (" +
                    // normalize_label(r.label, true)+")" +
                    // " --"+r.label+"-> " +
                    // sen.get(r.dependent).getForm() + " (" + normalize_label(r.label, false)+")");
                    // }
                    if (r.head == i && r.label.equals("Property"))
                        continue; // skip for later
                    if (r.head == i)
                        rel1 = normalize_label(r.label, true);
                    if (r.dependent == i)
                        rel1 = normalize_label(r.label, false);
                }
                for (Relation r : relations2) {
                    if (r.head == i && r.label.equals("Property"))
                        continue; // skip for later
                    if (r.head == i)
                        rel2 = normalize_label(r.label, true);
                    if (r.dependent == i)
                        rel2 = normalize_label(r.label, false);
                }
            ...
            for (Relation r : relations1) {
                instances.add(r.head);
                instances.add(r.dependent);
            }
            for (Relation r : relations2) {
                instances.add(r.head);
                instances.add(r.dependent);
            ...
            for (Relation r2 : relations2) {
                if (r.head == r2.head && r.dependent == r2.dependent) {
                    ...
                    if (r.label.equals(r2.label)) {
                        ...
                        // rel_disagreements++;
                        System.out.println("DISAGREEMENT: " + sen.get(r.head).getForm() + " --[" + r.label + " vs. "
                                + r2.label + "]--> " + sen.get(r.dependent).getForm());
                    }
                }
                if (r.head == r2.dependent && r.dependent == r2.head) {
                    // rel_disagreements++;
                    System.out.println("DISAGREEMENT: " + sen.get(r.head).getForm() + " <-- ?? --> "
        ...
        // System.out.printf("Raw agreement: %2f%n",
    ...
    private TreeMap<Integer, String> getPredicates(Sentence sen, SentenceAnnotation curr_anno)
    {
        ...
        for (String[] anno : curr_anno.conceptAnno) {
            ...
            if (currids.size() == 0) {
                ...
                // System.out.println(anno[2]+" "+ concept2word.get(anno[0]).getForm()); // Sanity
                // check
        ...
        for (String[] anno : curr_anno.relationAnno) {
            ...
            if (!l1.equals("Action") && (sen.get(arg1index).getHead().getPOS().equals("IN")/*
                                                                                            * ||
                                                                                            * sen.
                                                                                            * get
                                                                                            * (arg1index
            ...
            if (!l2.equals("Action") && (sen.get(arg2index).getHead().getPOS().equals("IN")/*
                                                                                            * ||
                                                                                            * sen.
                                                                                            * get
                                                                                            * (arg2index
            ...
            /**
             * Resolve disagreements first ** if(l1.equals("Action") && l2.equals("Object") &&
             * ((sen.get(arg2index).getPOS().equals("TO") ||
             * sen.get(arg2index).getPOS().equals("IN")) &&
             * !sen.get(arg2index).getChildren().isEmpty() )) arg2index =
             * sen.get(arg2index).getChildren().iterator().next().getIdx(); if(l1.equals("Object")
             * && ((sen.get(arg1index).getPOS().equals("TO") ||
             * sen.get(arg1index).getPOS().equals("IN")) &&
             * !sen.get(arg1index).getChildren().isEmpty() )) arg1index =
             * sen.get(arg1index).getChildren().iterator().next().getIdx(); /
             **/
