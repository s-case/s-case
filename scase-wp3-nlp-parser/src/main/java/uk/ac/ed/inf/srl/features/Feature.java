...
public abstract class Feature implements Serializable {
    ...
    public abstract void addFeatures(Sentence s,Collection<Integer> indices, Map<Integer, Double> nonbinFeats, int predIndex, int argIndex, Integer offset,boolean allWords); //This way the collection can be both a set or a list (ie one that can allow multiple identical values, or not, depending on choice from above)
    public abstract void addFeatures(Collection<Integer> indices, Map<Integer, Double> nonbinFeats, Predicate pred,Word arg,Integer offset,boolean allWords);
	...
	protected void addMap(String val){
		if(val!=null && !val.equals("") && !val.equals("_") && !indices.containsKey(val)){
	...
	//TODO this method should return null rather than -1 if its missing. I assume its faster to test if its null than all the autoboxing that goes on when you compare an Integer to -1
