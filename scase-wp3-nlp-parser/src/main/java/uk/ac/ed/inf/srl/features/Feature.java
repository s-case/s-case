        ...
        implements Serializable
{
    ...
    public abstract void addFeatures(Sentence s, Collection<Integer> indices, Map<Integer, Double> nonbinFeats,
            int predIndex, int argIndex, Integer offset, boolean allWords); // This way the
                                                                            // collection can be
                                                                            // both a set or a list
                                                                            // (ie one that can
                                                                            // allow multiple
                                                                            // identical values, or
                                                                            // not, depending on
    ...
    public abstract void addFeatures(Collection<Integer> indices, Map<Integer, Double> nonbinFeats, Predicate pred,
    ...
    // TODO this method should return null rather than -1 if its missing. I assume its faster to
    // test if its null than all the autoboxing that goes on when you compare an Integer to -1
    ...
    protected boolean doExtractFeatures(Word pred)
    {
        return pred.getPOS().startsWith(POSPrefix)
                || (usedForPredicateIdentification && !Learn.learnOptions.skipNonMatchingPredicates && pred instanceof Predicate);
    }
