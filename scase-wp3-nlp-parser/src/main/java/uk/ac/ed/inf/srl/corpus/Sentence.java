        ...
        extends ArrayList<Word>
{
    ...
    public String toSpecialString()
    {
        ...
        for (int i = 1; i < super.size(); ++i) {
            Word w = super.get(i);
            ret.append(i).append("\t").append(w.toSpecialString());
            if (!(w instanceof Predicate)) // If its not a predicate add the FILLPRED and PRED cols
                ret.append("\t_\t_");
            for (int j = 0; j < predicates.size(); ++j) {
                ret.append("\t");
    ...
    /*
     * Functions used when interfacing with Bohnets parser These need to be fixed. Or rather the
     * Sentence object should go altogether.
     */
    ...
    public static Sentence newSRLOnlySentence(String[] lines)
    {
        ...
        for (String line : lines) {
            ...
            if (!line.matches("^[0-9].*")) {
                ...
                corefid = cols[15 + offset].equals("_") ? null : cols[15 + offset];
        ...
        for (String line : lines) {
            ...
            for (int i = 14; i < cols.length; i++) {
                if (cols[i].charAt(0) == 'Y') {
                    // ret.getPredicates().get(i-14).addArgMap(ret.get(ix), "ARG");
                }
            }
            ix++;
        }
