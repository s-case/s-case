...
public class ArgMap extends HashMap<Word,String>{
	...
	public double computeScore(Map<Word,String> goldStandard){
		if(this.size()==0 && goldStandard.size()==0) //If both are empty its 1
			return 1;
		int tp=0,fp=0,fn=0;
		for(Word w:this.keySet()){
			if(goldStandard.containsKey(w)){
				...
				if(goldLabel.equals(propLabel)){ //Match
					...
					++fn; //Correct Label non-NONE, guessed Label non-NONE but incorrect
					++fp; //Guessed label non-NONE, correct Label different non-NONE
				}
	...
	public String toString(){
		return "Prob: "+prob+", IDProb: "+idProb+", LblProb: "+lblProb+", RerankProb: "+rerankProb+". "+super.toString();
	}
