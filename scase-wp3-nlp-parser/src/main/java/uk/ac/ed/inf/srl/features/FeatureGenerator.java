...
public class FeatureGenerator implements Serializable {
    ...
    public Map<Step,FeatureSet> readFeatureFiles(Map<Step,File> files,BrownCluster bc,WordEmbedding we) throws IOException {
    ...
    private FeatureSet createFeatureSet(Map<String,List<String>> names,boolean includeAllWords,BrownCluster bc, WordEmbedding we){
    ...
    public Feature getFeature(String featureNameString,boolean includeAllWords,String POSPrefix,BrownCluster bc, WordEmbedding we){
    ...
    public Feature getFeature(FeatureName fn,boolean includeAllWords,String POSPrefix,BrownCluster bc, WordEmbedding we){
		Feature ret;
		if(cache.containsKey(fn)){
			...
			case OntPredLemmaSense:	ret=new PredDependentAttrFeature(fn,WordData.OntPred,TargetWord.Pred,false,POSPrefix); break;
			...
			case PredSubjWord:		ret=new PredDependentAttrFeature(fn,WordData.Form,TargetWord.PredSubj,includeAllWords,POSPrefix); break;
			case PredSubjPOS:		ret=new PredDependentAttrFeature(fn,WordData.POS,TargetWord.PredSubj,includeAllWords,POSPrefix); break;
			...
			case POSPath:			ret=new PathFeature(fn,WordData.POS,false,POSPrefix); break;
			case POSDepPath:		ret=new PathFeature(fn,WordData.POS,true,POSPrefix); break;
			//case POSPath:			ret=new PathFeature(fn,WordData.POS,POSPrefix); break;
			case DeprelPath:		ret=new PathFeature(fn,WordData.Deprel,true,POSPrefix); break;
			case Position:			ret=new PositionFeature(POSPrefix); break;
			case Distance:			ret=new DistanceFeature(fn,null,true, POSPrefix); break;
			case AssignedPBLabel:	ret=new PBLabelFeature(fn, TargetWord.Arg,includeAllWords,POSPrefix); break;
			
			case Coref:				ret=new CorefFeature(POSPrefix); break;
			
			default: 
				if(fn.toString().startsWith("WordEmbedding") || fn.toString().startsWith("WordTokenEmbedding")) {
					if(we==null) {
					    throw new RuntimeException("Cannot use embedding features unless embeddings are provided on the cmd line");
					}
					...
					if(fn.toString().contains("Path")){
					  ...
					  if(fn.toString().contains("Pred")) {
						  if(fn.toString().contains("Token"))
							  ret=new PredDependentEmbedding(fn, TargetWord.valueOf("Pred"), includeAllWords, POSPrefix, we, dim, true);
						  else
							  ret=new PredDependentEmbedding(fn, TargetWord.valueOf("Pred"), includeAllWords, POSPrefix, we, dim, false);
					  } else if(fn.toString().contains("Subj")) {
						  if(fn.toString().contains("Token"))
							  ret=new PredDependentEmbedding(fn, TargetWord.PredSubj, includeAllWords, POSPrefix, we, dim, true);
						  else
							  ret=new PredDependentEmbedding(fn, TargetWord.PredSubj, includeAllWords, POSPrefix, we, dim, false);
					  }
					  else
					  {
						  if(fn.toString().contains("Comp")) {
							  if(fn.toString().contains("Token")) {
								  TargetWord tw=TargetWord.valueOf(fn.toString().substring(22, fn.toString().length()-3));						  
								  ret=new ArgDependentEmbedding(fn, tw, POSPrefix, true, false, we, dim, true);							  
							  } else {
								  TargetWord tw=TargetWord.valueOf(fn.toString().substring(17, fn.toString().length()-3));						  
								  ret=new ArgDependentEmbedding(fn, tw, POSPrefix, true, false, we, dim, false);
							  }
						  } else if(fn.toString().contains("Avg")) {
							  if(fn.toString().contains("Token")) {
								  TargetWord tw=TargetWord.valueOf(fn.toString().substring(21, fn.toString().length()-3));						  
								  ret=new ArgDependentEmbedding(fn, tw, POSPrefix, false, true, we, dim, true);							  
							  } else {
								  TargetWord tw=TargetWord.valueOf(fn.toString().substring(16, fn.toString().length()-3));
    ...
    public Feature getQFeature(FeatureName fn1,FeatureName fn2,boolean includeAllWords,String POSPrefix,BrownCluster bc, WordEmbedding we){
		Feature ret;
		...
		if(f1 instanceof SingleFeature){
		    ...
		    if(f2 instanceof SingleFeature) {
			ret=new QContinuousSingleFeature((ContinuousFeature) f1,(SingleFeature) f2, includeAllWords, POSPrefix);
			...
			throw new IllegalArgumentException("Features " + f1.getName()+" and "+f2.getName()+" cannot be combined. Change your feature file");
		    }
		} else {
			if(f2 instanceof SingleFeature){
