        ...
        extends FullPipelineOptions
{
    ...
    // TODO add some sort of toString() method to all option classes. And make the system print
    // these out when it is initialized.
    ...
    int trySubParseArg(String[] args, int ai)
    {
        if (args[ai].equals("-out")) {
            ...
            ai++;
            skipAI = true;
        } else if (args[ai].equals("-desegment")) { // Not printed out in the help
                                                    // (getSubUsageOptions()), don't think it needs
                                                    // to. This is only experimental.
            ai++;
            desegment = true;
            skipPI = false; // This won't be regarded anyway. It's not applicable when the initial
