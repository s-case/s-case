...
import uk.ac.ed.inf.srl.http.AbstractPipeline.SentenceTooLongException;
        ...
        extends AbstractHandler
{
            ...
            throws IOException
    {
        ...
        if (inputSentence == null || inputSentence.length() == 0
                || WHITESPACE_ONLY_PATTERN.matcher(inputSentence).matches()) {
            sendContent(exchange, "Error, invalid request.", "text/plain", 400);
            return;
        }

        String userAgent = AbstractHandler.getHeader(exchange, "User-agent");
        String xForwardedFor = AbstractHandler.getHeader(exchange, "X-forwarded-for"); // Header
                                                                                       // provided
                                                                                       // by apache
                                                                                       // mod_proxy
        ...
        System.out.println("Requested by " + exchange.getRemoteAddress()
                + (xForwardedFor == null ? "" : " (Forwarded for " + xForwardedFor + ")") + ". Agent: " + userAgent);
