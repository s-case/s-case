        ...
        extends SemanticRoleLabeler
{
    ...
    private Pipeline(FeatureGenerator fg, PredicateReference predicateReference, List<String> argLabels,
        ...
        for (Step step : Step.values()) {
            if (featureSets.containsKey(step)) {
                ...
                case pi:
                    ...
                    break;
                case pd:
                    steps.add(new PredicateDisambiguator(featureSets.get(Step.pd), predicateReference));
                    break;
                case ai:
                    steps.add(new ArgumentIdentifier(featureSets.get(Step.ai)));
                    break;
                case ac:
                    steps.add(new ArgumentClassifier(featureSets.get(Step.ac), argLabels));
                    break;
                // case po: steps.add(new PredicateConverter(featureSets.get(Step.po))); break;
                // case ao: steps.add(new
                // ArgumentConverter(featureSets.get(Step.ao),convArgLabels)); break;
                }
            }
        }
    }
    ...
    public static void extractLabels(Iterable<Sentence> reader, PredicateReference predicateReference,
    ...
    public static Pipeline trainNewPipeline(Iterable<Sentence> sentences, FeatureGenerator fg, ZipOutputStream zos,
            Map<Step, FeatureSet> featureSets)
            throws IOException
    {
    ...
    public static Pipeline trainNewPipeline(Iterable<Sentence> sentences, Map<Step, File> featureFiles,
            ZipOutputStream zos, BrownCluster bc, WordEmbedding we)
            throws IOException
    {
        ...
        Map<Step, FeatureSet> featureSets = fg.readFeatureFiles(featureFiles, bc, we);
            ...
            throws IOException, ClassNotFoundException
    {
        ...
        for (Step s : steps) {
            BufferedReader in = new BufferedReader(new InputStreamReader(zipFile.getInputStream(zipFile
                    .getEntry(LearnOptions.featureFileNames.get(s)))));
    ...
    public static Pipeline fromZipFile(ZipFile zipFile)
