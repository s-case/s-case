	...
	private void setup(Map<Step,FeatureSet> featureSets,PredicateReference predicateReference,List<String> argLabels){
		...
		for(Step step:Step.values()){
			if(featureSets.containsKey(step)){
				...
				//case ao: steps.add(new ArgumentConverter(featureSets.get(Step.ao),convArgLabels)); break;
				}
			}
		}
	}
...
//	}
    ...
    public static Pipeline trainNewPipeline(Iterable<Sentence> sentences,Map<Step,File> featureFiles,ZipOutputStream zos,BrownCluster bc, WordEmbedding we) throws IOException {
	...
	public static Pipeline fromZipFile(ZipFile zipFile) throws ZipException, IOException, ClassNotFoundException {
	    return fromZipFile(zipFile,new Step[]{Step.pi,Step.pd,Step.ai,Step.ac/*,Step.po,Step.ao*/});
	}
