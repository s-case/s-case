        ...
        implements Serializable
{
     ...
     * 
     * @param in the inputstream, assuming header is parsed and weights are next
     * @param features number of features
     * @param classes number of classes
     * @param w the w-string (last line before the weights), should be either 'w' or 'w-sparse'
     * @return the appropriate weightvector
     */
    public static WeightVector parseWeights(BufferedReader in, int features, int classes, double bias, boolean sparse)
            throws IOException
    {
        if (sparse) {
            if (classes == 2)
    ...
    public abstract double[] computeAllProbs(Collection<Integer> ints, Map<Integer, Double> nonbinFeats);

    public abstract short computeBestClass(Collection<Integer> ints, Map<Integer, Double> nonbinFeats);

    // For binary classifiers
    public abstract static class BinaryVector
            extends WeightVector
    {
        ...
        protected abstract double computeScore(Collection<Integer> ints, Map<Integer, Double> nonbinFeats);

        public double[] computeAllProbs(Collection<Integer> ints, Map<Integer, Double> nonbinFeats)
        {
            double[] ret = new double[2];
            double prob = (1.0 / (1.0 + Math.exp(-computeScore(ints, nonbinFeats))));
            ret[0] = prob;
            ret[1] = 1.0 - prob;
            return ret;
        }

        public short computeBestClass(Collection<Integer> ints, Map<Integer, Double> nonbinFeats)
        {
            ...
            extends BinaryVector
    {
        ...
        protected double computeScore(Collection<Integer> ints, Map<Integer, Double> nonbinFeats)
        {
            ...
            extends BinaryVector
    {
        ...
        protected double computeScore(Collection<Integer> ints, Map<Integer, Double> nonbinFeats)
        {
            ...
            for (Integer i : nonbinFeats.keySet()) {
                if ((i - 1) < features && weightMap.containsKey(i - 1) && nonbinFeats.get(i - 1) != null) {
    ...
    public abstract static class MultipleVector
            extends WeightVector
    {
        private static final long serialVersionUID = 1L;
        ...
        protected abstract double[] computeScores(Collection<Integer> ints, Map<Integer, Double> nonbinFeats);
            ...
            extends MultipleVector
    {
        ...
        protected double[] computeScores(Collection<Integer> ints, Map<Integer, Double> nonbinFeats)
        {
            double[] ret = new double[classes];
        ...
        protected double[] computeScores(Collection<Integer> ints, Map<Integer, Double> nonbinFeats)
        {
            ...
            for (int i = 0; i < classes; ++i) {
                double curvalue = bias > 0 ? (weightMap.containsKey(features) ? bias
                        * weightMap.get(features).weights[i] : 0d) : 0d;
                for (Integer in : ints) {
                ...
                for (Integer in : nonbinFeats.keySet()) {
                    if (weightMap.containsKey(in - 1) && (in - 1) < features)
                        curvalue += weightMap.get(in - 1).weights[i] * nonbinFeats.get(in);
                }
                ret[i] = curvalue;
            }
            return ret;
