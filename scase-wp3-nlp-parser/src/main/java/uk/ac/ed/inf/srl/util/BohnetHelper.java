...
import uk.ac.ed.inf.srl.options.Options;
...
public class BohnetHelper
{
    ...
    public static is2.mtag.Tagger getMTagger(File modelFile)
            throws IOException
    {
    ...
    {
        String[] argsDP = { "-model", modelFile.toString(), "-cores",
                Integer.toString(Math.min(Options.cores, FullPipelineOptions.cores)) };
