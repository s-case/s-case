...
import uk.ac.ed.inf.srl.CompletePipeline;
import uk.ac.ed.inf.srl.corpus.Predicate;
import uk.ac.ed.inf.srl.corpus.Sentence;
import uk.ac.ed.inf.srl.corpus.Word;
import uk.ac.ed.inf.srl.corpus.Yield;
import uk.ac.ed.inf.srl.http.ParseRequestHandler.StringPair;
import uk.ac.ed.inf.srl.languages.Language;
import uk.ac.ed.inf.srl.languages.Language.L;
import uk.ac.ed.inf.srl.options.HttpOptions;
import uk.ac.ed.inf.srl.util.FileExistenceVerifier;
import uk.ac.ed.inf.srl.util.Sentence2RDF;

public class SRLHttpPipeline
        extends AbstractPipeline
{
    ...
    private static final String STYLESHEET = "<style type=\"text/css\">%n" + "  table { background-color:#000000 }%n"
            + "  td { background-color: #EEEEEE}%n" + "  th { background-color: #EEEEEE}%n"
            + "  .topRowCell {border-bottom: 1px solid black}%n" + "  .A0, .C-A0 {background-color:#CCCC00}%n"
            + "  .A1, .C-A1 {background-color:#CC0000}%n" + "  .A2, .C-A2 {background-color:#00CC00}%n"
            + "  .A3, .C-A3 {background-color:#0000CC}%n" + "  .AM-NEG {background-color:#CC00CC}%n"
            + "  .AM-MNR {background-color:#00CCCC}%n" + "  .ARG_DEFAULT {background-color:#CCCCCC}%n" + "</style>%n";

    // TODO improve the color codes for different labels, and across languages. I.e. refactor this
    // and put it in the Language classes, rather than here.
    private static final String HTMLHEAD = "<html><head>%n<title>Semantic Role Labeler Demo</title>%n" + STYLESHEET
            + "</head>%n<body>%n";
    ...
    {
        String s = "  <h3>Try the semantic role labeler</h3>%n" + "  Enter a sentence in <b>"
                + Language.getLanguage().toLangNameString()
                + "</b> and press Parse.<br/>%n"
                ...
                + "\" rows=\"3\" cols=\"40\"></textarea></td></tr>%n"
                ...
                + "      <tr><td colspan=\"2\"><input type=\"checkbox\" name=\"doPerformDictionaryLookup\" /> <font size=\"-1\">Attempt to lookup and reference predicates in dictionary<sup>&dagger;</sup>.</font></td></tr>%n"
                ...
                + "      <li>Semantic Role Labeling - based on LTH's contribution to the CoNLL 2009 ST</li>%n"
                ...
                + "  <font size=\"-1\"><sup>&dagger;</sup> This is only applicable for HTML response, and with English. Note that this takes longer, and if the online dictionary is down, it may time out and take a significant amount of time.</font>%n";
        return s;
    }

    @Override
    public StringPair parseRequest(String inputSentence, Map<String, String> vars)
            throws Exception
    {
    ...
    private String getHTMLResponse(Sentence sen, long parsingTime, boolean performURLLookup,
            boolean includeDependencyGraphImage)
    {
        StringBuilder ret = new StringBuilder(HTMLHEAD);
        ret.append("%n").append(super.HTML_TOP_EXTRA);
        // ret.append("<html><head><title>Semantic Parser</title>%n"+STYLESHEET+"</head><body>%n");
        ret.append("<table cellpadding=10 cellspacing=1>%n<tr><td class=\"topRowCell\">&nbsp;</td>");
        ...
        for (Predicate pred : sen.getPredicates()) {
            ...
            for (Yield y : yields) {
                if (!y.isContinuous()) { // Warn the user if we have discontinuous yields
                    errors.append("((Discontinous yield of argument '" + y + "' of predicate '" + pred.getSense()
                ...
                if (blankColSpan > 0) {
                    ...
                    errors.append("Argument '" + y.getArgLabel() + "' of '" + pred.getSense() + "' at index "
                            + indexCount + " overlaps with previous argument(s), ignored.%n");
                    continue;
                }
                int argColWidth = sen.indexOf(y.last()) - sen.indexOf(y.first()) + 1;
                String argLabel = y.getArgLabel();
                ret.append("<td colspan=\"").append(argColWidth).append("\" class=\"")
                        .append((styleSheetArgs.contains(argLabel) ? argLabel : "ARG_DEFAULT"))
                        .append("\" align=\"center\">").append(argLabel).append("</td>");
        ...
        ret.append("%n</table><br/>%n");
        ret.append("Parsing sentence required " + parsingTime + "ms.<br/>%n");
        if (errors.length() > 0) {
            ret.append("<br/><hr><br/><font color=\"#FF0000\">Errors</font><br/>");
            ret.append(errors.toString().replace("%n", "<br/>"));
            System.err.println(errors.toString().trim());
        }
        ret.append("<br/>%n<hr/>%n<br/>%n");
        // The dependency graph image
        /*
         * temporarily removed (MR, 23 Oct 2014) if(includeDependencyGraphImage){ try {
         * ByteArrayOutputStream
         * baos=WhatsWrongHelper.renderPNG(WhatsWrongHelper.getNLPInstance(sen),1); String
         * key=imageCache.addObject(baos); ret.append("<img src=\"/img/"+key+".png\"/>");
         * ret.append("<br/>%n<hr/>%n<br/>%n"); } catch (IOException e) { e.printStackTrace(); }
         * 
         * }
         */
    ...
    /**
     * This method tries to make an HTTP request to the given URL and if it works and the request is
     * OK (i.e. return code 200), it returns true. Otherwise false.
     * 
     * @param url the url
            ...
            throws ZipException, IOException, ClassNotFoundException
    {
        ...
        HttpPipeline.setupHttpPipeline(options, new SRLHttpPipeline(completePipeline, HttpPipeline.imageCache,
                options.l, options.sentenceMaxLength, options));
