...
public class Reranker extends SemanticRoleLabeler {
	...
	public Reranker(ParseOptions parseOptions) throws ZipException, IOException, ClassNotFoundException{
		...
		pipeline=parseOptions.skipPD ? Pipeline.fromZipFile(zipFile, new Step[]{Step.ai,Step.ac}) : noPI ? Pipeline.fromZipFile(zipFile, new Step[]{Step.pd,Step.ai,Step.ac}) : Pipeline.fromZipFile(zipFile);
...
//			sumProbs+=prob;
    ...
    private Collection<Integer> collectPipelineFeatureIndices(Predicate pred,ArgMap argMap,Collection<Integer> indices, Map<Integer, Double> nonbinFeats) {
	 ...
	 */
    private void addAndCollectGlobalFeatures(Predicate pred,ArgMap argMap,Collection<Integer> indices, Map<Integer, Double> nonbinFeats) {
	 ...
	 * @param indices the container to add the index too
	 */
    private void collectGlobalFeatures(Predicate pred, ArgMap argMap, Collection<Integer> indices, Map<Integer, Double> nonbinFeats) {
		String cals=Language.getLanguage().getCoreArgumentLabelSequence(pred, argMap);
		Integer index=calsMap.get(cals); //Need to build this beforehand -- can't be done, since we don't know what faulty CALS we will generate during training
		if(index!=null)
		    indices.add(sizePipelineFeatures+index);
