...
import uk.ac.ed.inf.srl.SemanticRoleLabeler;
        ...
        extends SemanticRoleLabeler
{
    ...
    public Reranker(ParseOptions parseOptions) throws ZipException, IOException, ClassNotFoundException {
        ...
        pipeline = parseOptions.skipPD ? Pipeline.fromZipFile(zipFile, new Step[] { Step.ai, Step.ac })
                : noPI ? Pipeline.fromZipFile(zipFile, new Step[] { Step.pd, Step.ai, Step.ac }) : Pipeline
                        .fromZipFile(zipFile);
    ...
    // private static Reranker fromZipFile(ZipFile zipFile,boolean noPI,double alfa,int aiBeam,int
    // acBeam) throws ZipException, IOException, ClassNotFoundException{
    // Pipeline pipeline= noPI ? Pipeline.fromZipFile(zipFile,new Step[]{Step.pd,Step.ai,Step.ac}) :
    ...
    // @SuppressWarnings("unchecked")
    // private Reranker(Pipeline pipeline,ZipFile zipFile, boolean noPI, double alfa,int aiBeam,int
    // acBeam) throws IOException, ClassNotFoundException {
    ...
    public Reranker(LearnOptions learnOptions, ZipOutputStream zos) throws IOException {
        this(1.0, false, learnOptions.global_aiBeam, learnOptions.global_acBeam); // 1.0 and false
                                                                                  // make no
                                                                                  // difference
                                                                                  // during
                                                                                  // training.
        // Start by training a usual pipeline, then drag the featuregenerator and argLabels out of
        // it and reuse them.
        ...
        BrownCluster bc = Learn.learnOptions.brownClusterFile == null ? null : new BrownCluster(
                Learn.learnOptions.brownClusterFile);
        WordEmbedding we = Learn.learnOptions.wordEmbeddingFile == null ? null : new WordEmbedding(
                Learn.learnOptions.wordEmbeddingFile);
        Pipeline fullPipeline = Pipeline.trainNewPipeline(trainCorpus, learnOptions.getFeatureFiles(), zos, bc, we);
        FeatureGenerator fg = fullPipeline.getFg();
        argLabels = fullPipeline.getArgLabels();
        Map<Step, FeatureSet> featureSets = new HashMap<Step, FeatureSet>(fullPipeline.getFeatureSets()); // We
                                                                                                          // copy
                                                                                                          // this
                                                                                                          // and
                                                                                                          // use
                                                                                                          ...
                                                                                                          // reranker
                                                                                                          // subclassifiers
        featureSets.remove(Step.pi);// This way we will not be training a bunch of superfluous
                                    // training of the PI module.
        fullPipeline = null; // And we're done with the full pipeline. Let the garbage collector
                             // handle it
        ...
        for (int i = 0; i < subCorpora.size(); ++i) {
            ...
            ArgumentIdentifier aiModule = (ArgumentIdentifier) pipeline.steps.get(1); // Note since
                                                                                      // we have no
                                                                                      // PI,
            ArgumentClassifier acModule = (ArgumentClassifier) pipeline.steps.get(2); // the AI and
                                                                                      // AC modules
                                                                                      // are shifted
                                                                                      // down one
                                                                                      // step.
            ...
            for (Sentence sen : testCorpus) {
                for (Predicate pred : sen.getPredicates()) {
                    ++predCount;
                    // Do the beam search
                    List<ArgMap> negatives = acModule.beamSearch(pred,
                            aiModule.beamSearch(pred, learnOptions.global_aiBeam), learnOptions.global_acBeam);
                    ...
                    // Ok, we now have a number of positive examples in bestArgMaps, and the
                    // negative ones in argMaps
                    ...
                    for (ArgMap am : positives) {
                        ...
                        collectPipelineFeatureIndices(pred, am, indices, nonbinFeats);
                        addAndCollectGlobalFeatures(pred, am, indices, nonbinFeats);
                        Collections.sort((List<Integer>) indices);
                        lp.addInstance(AbstractStep.POSITIVE, indices, nonbinFeats);
                    }
                    for (ArgMap am : negatives) {
                        ...
                        addAndCollectGlobalFeatures(pred, am, indices, nonbinFeats);
                        Collections.sort((List<Integer>) indices);
                        lp.addInstance(AbstractStep.NEGATIVE, indices, nonbinFeats);
                    }
                }
            }
        }
    ...
    private Collection<Integer> collectPipelineFeatureIndices(Predicate pred, ArgMap argMap,
            Collection<Integer> indices, Map<Integer, Double> nonbinFeats)
    {
        for (Word arg : argMap.keySet()) {
            Integer aiOffset = 0;
            HashSet<Integer> currentInstance = new HashSet<Integer>(); // I think we need to watch
                                                                       // out for doubles here.
    ...
    /**
     * This method extracts the one global feature (core argument label sequence). It also adds new
     * features to the map. The method is meant to be run during training.
     * 
            ...
            Map<Integer, Double> nonbinFeats)
    {
        ...
        Integer index = calsMap.get(cals); // Need to build this beforehand -- can't be done, since
                                           // we don't know what faulty CALS we will generate during
                                           // training
        if (index == null) {
    ...
    /**
     * This method extract the one global feature (core argument label sequence). It is meant to be
     * used during parsing and does not add new features.
     * 
     * @param pred the predicate
    ...
    private static double partitionBestArgMaps(List<ArgMap> candidates, Map<Word, String> goldStandard,
            Set<ArgMap> bestArgMaps)
    {
        double bestScore = 0;
    ...
    private static List<List<Sentence>> partitionCorpus(Iterable<Sentence> sentences, int numberOfPartitions)
    {
        ...
        if (Learn.learnOptions.deterministicReranker) {
            ...
            for (Sentence s : sentences) {
                // if(s.getPredicates().size()==0) //TODO uncomment this and verify that we get the
                // same figures (roughly, seeing as no two rerankers come out identical anyway)
                // continue;
